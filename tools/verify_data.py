...
"""
import sys, os, re, glob, subprocess, tempfile, argparse
...
def main():
    ...
    parser.add_argument("--cmake-output",
                        help="write an explicit CMake list containing only byte-exact inputs")
    ...
    with tempfile.TemporaryDirectory() as td:
        for f in files:
            # Both naming generations are durable repository inputs:
            # rodata_0x271d8.c and the older rodata_19934.c form.
    ...
    if args.cmake_output and (mism or skip):
        raise SystemExit("refusing CMake list: %d mismatched, %d skipped" %
                         (mism, skip))
    if args.cmake_output:
        lines = [
            "# Generated by tools/verify_data.py --cmake-output; do not edit.",
            "# Byte-exact inputs: %d files, %d bytes." % (len(verified_files), matched_bytes),
            "set(G1_VERIFIED_DATA_SOURCES",
        ]
        for path in verified_files:
            # The generated file is consumed from recon/generated.  Keep its
            # content independent of the caller's temporary output path so it
            # can be regenerated and compared deterministically.
            lines.append('  "${CMAKE_CURRENT_LIST_DIR}/../data/%s"' %
                         os.path.basename(path))
        lines.append(")")
        with open(args.cmake_output, "w") as output:
            output.write("\n".join(lines) + "\n")
