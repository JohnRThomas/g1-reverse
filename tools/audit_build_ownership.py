...
def parse_map(text: str, object_sources: dict[str, str], evidence: str) -> list[Definition]:
    ...
    for line in text.splitlines():
        ...
        if section:
            ...
            symbol = section.group("section_symbol")
            if symbol:
                source = _source_for_origin(current_origin, object_sources)
                definitions.append(Definition(symbol, current_origin,
...
def _expected_units(entry: dict) -> set[str]:
    ...
    if unit and not unit.startswith(".text "):
        units.add(os.path.basename(unit))
    for item in entry.get("evidence", []):
        for key in ("member", "object", "unit"):
            if item.get(key):
                units.add(os.path.basename(str(item[key])))
    return units
...
def main(argv: list[str] | None = None) -> int:
    ...
    if command_objects:
        found, undef = parse_nm(_run_nm_batched(args.nm, command_objects), object_sources,
                                "compile_commands objects")
        definitions.extend(found); undefined.update(undef); evidence_files.extend(command_objects)
    ...
    if existing_objects:
        ...
        definitions.extend(found); undefined.update(undef); evidence_files.extend(existing_objects)
