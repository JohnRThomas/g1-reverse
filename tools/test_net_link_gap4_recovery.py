#!/usr/bin/env python3
"""Fail-closed gates for four retained private CPUNET link targets."""
...
import build_net_link_gap4_recovery as builder
import cfg_verify


ROOT = Path(__file__).resolve().parents[1]
RECEIPT = ROOT / "recon/ownership/net_link_gap4_recovery.json"


class NetLinkGap4RecoveryTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_receipt_replays_authoritative_proofs(self):
        ...
        self.assertEqual(0x190, rebuilt["summary"]["total_true_executable_extent"])
        self.assertEqual(19, rebuilt["summary"]["cfg_directed_cover_cases"])
    ...
    def test_boundary_mutation_is_rejected(self):
        ...
        try:
            ...
            source = (ROOT / "recon/net/src/FUN_0102722c.c").read_text()
            mutated = source.replace("payload_size + 8u", "payload_size + 7u")
            verdict = cfg_verify.verify(
                "net", "FUN_0102722c", trials_random=0, source_override=mutated)
