...
# Allow Kconfig warnings (don't abort on warnings)
...
# Network - basic networking only to avoid IRQ conflicts
CONFIG_NETWORKING=y
CONFIG_NET_BUF=y
# Disable advanced networking to prevent IRQ conflicts
# CONFIG_NET_IPV4=y
# CONFIG_NET_IPV6=y
# CONFIG_NET_TCP=y
# CONFIG_NET_UDP=y
...
# GPIO and peripherals - enable all
CONFIG_GPIO=y
CONFIG_SERIAL=y
...
CONFIG_CRYPTO=y
# CONFIG_CRYPTO_TINYCRYPT=y     # Undefined symbol
...
# ARM specific - enable all features
CONFIG_ARM_MPU=y
...
# Power management
# CONFIG_PM=y                   # Unsatisfied dependencies
# CONFIG_PM_DEVICE=y            # Depends on PM
...
# Shell - for maximum code inclusion
CONFIG_SHELL=y
CONFIG_SHELL_BACKENDS=y
CONFIG_SHELL_BACKEND_SERIAL=y

# Device drivers - enable many
# CONFIG_DEVICE_POWER_MANAGEMENT=y  # Undefined symbol

# Enable Nordic-specific subsystems for target compilation units
# CONFIG_ESB=y
# CONFIG_MPSL=y

# Enable basic Bluetooth to avoid IRQ conflicts
CONFIG_BT=y
CONFIG_BT_HCI=y
# CONFIG_BT_HCI_HOST=y
# Disable advanced BT features to prevent IRQ conflicts
...
# Enable IPC for dual-core communication (needed for nrfx drivers)
CONFIG_MBOX=y
CONFIG_IPM=y
CONFIG_IPM_NRFX=y
...
# Enable work queues and scheduling
...
# Test framework - disabled due to main() function conflict
# CONFIG_ZTEST=y
...
# Build as static library to keep all functions
CONFIG_BUILD_OUTPUT_BIN=n
CONFIG_BUILD_OUTPUT_STRIPPED=n

# --- fix HCI-RPMsg build ---
CONFIG_MBOX=y               # generic mailbox support
CONFIG_MBOX_NRFX_IPC=y      # Nordic IPC-based mailbox driver
CONFIG_NRFX_IPC=y           # (selected automatically, keep explicit)

# --- avoid a second IRQ-42 registration ---
CONFIG_IPM=n                # generic IPM layer still present but unused
CONFIG_IPM_NRFX=n           # disable Nordic IPM driver
# ── fix link error caused by missing mbox_bt device ─────────────
CONFIG_BT_RPMSG=n          # don’t build HCI-RPMsg driver
CONFIG_BT_HCI=n            # no alternative HCI transport needed
CONFIG_BT_CTLR=n           # keep controller off on application core
