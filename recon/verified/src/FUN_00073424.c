...
extern void FUN_0008656a(volatile uint32_t *delayable_work);
...
int FUN_00073424(uint32_t queue, volatile uint32_t *delayable_work,
                 uint32_t delay_low, uint32_t delay_high)
{
    ...
    if (delayable_work == 0) {
        FUN_0007e2fa(0x00099cbdUL, 0x000f82dfUL, 0x000f820fUL, 0x3deUL);
        FUN_0007e2ec(0x000f820fUL, 0x3deUL);
    }
    ...
    FUN_0008656a(delayable_work);
    if ((delay_low | delay_high) == 0U) {
        result = FUN_00072cd4(delayable_work, &queue, 0U,
                              delayable_work[3]);
    } else {
