/* Reconstructed slave_display_thread @ 0x27cfe true_code_size=2554 (0x27cfe..0x286f8, incl. interleaved literal pools).
   Parity: 297/300 no_return trials PASS (seed0/trials=1 PASS). 3 seeds fail on a Unicorn
   harness artifact (spurious extra code-hook fire at change_work stub+2 in case 2); the
   reconstruction reproduces the original disassembly instruction-for-instruction there.
   NOTE: Ghidra size 2278 UNDERSTATED the function; no trailing data table (literal pools
   are interleaved and end exactly where the next function prologue begins at 0x286f8). */
#include <stdint.h>
