/* Reconstructed FUN_000564cc @ 0x000564cc (bt_conn_send_cb).
 * Raw/address backmap: FUN_000564cc @ 0x000564cc, extent 0x000000de.
 */
#include <stdint.h>

extern void FUN_000813ca(uint32_t, uint32_t, const void *);
extern void *FUN_000748ac(void);                 /* k_current_get */
extern void *FUN_000727ac(void *, uint32_t, uint32_t, uint32_t); /* k_fifo_get */
extern void FUN_00086502(void *, void *);        /* k_fifo_put */
extern void FUN_0005f200(void *, void *);        /* net_buf_put */

struct bt_conn_tx_recovered {
    uint32_t node;
    void (*callback)(void *, void *, uint8_t);
    void *user_data;
    uint32_t pending_no_callback;
};

int FUN_000564cc(uint8_t *connection, uint8_t *buffer,
                 void (*callback)(void *, void *, uint8_t), void *user_data)
{
    if (buffer[0x0b] < 8U) {
        uint32_t package[3] = { 4U, 0x000f3badU, buffer[0x0b] };
        FUN_000813ca(0x00088108U, 0x2040U, package);
        return -22;
    }
    if (connection[0x0d] != 7U) {
        uint32_t package[2] = { 2U, 0x000f3bd2U };
        FUN_000813ca(0x00088108U, 0x1040U, package);
        return -128;
    }

    struct bt_conn_tx_recovered *tx = 0;
    if (callback != 0) {
        void *current = FUN_000748ac();
        volatile uint32_t *const system_workqueue_thread =
            (volatile uint32_t *)0x20003a60U;
        uint32_t timeout = current == (void *)*system_workqueue_thread ? 0U : UINT32_MAX;
        tx = FUN_000727ac((void *)0x200068d0U, 0U, timeout, timeout);
        if (tx == 0)
            return -105;
        if (connection[0x0d] != 7U) {
            uint32_t package[2] = { 2U, 0x000f3be1U };
            FUN_000813ca(0x00088108U, 0x1080U, package);
            tx->callback = 0;
            tx->user_data = 0;
            tx->pending_no_callback = 0;
            FUN_00086502((void *)0x20003a60U, tx);
            return -128;
        }
        tx->callback = callback;
        tx->user_data = user_data;
        tx->pending_no_callback = 0;
    }

    *(struct bt_conn_tx_recovered **)(buffer + 0x18) = tx;
    buffer[0x1c] = 0;
    FUN_0005f200(connection + 0x38, buffer);
    return 0;
}
