/* Reconstructed set_device_sn_info @ 0x32164  (parity: 300/300 trials, PROVEN) */

#include <stdint.h>
extern void DEBUG_PRINT(unsigned long, ...);
extern int FUN_000167a8(void);
extern void FUN_00019c70(unsigned long, ...);
extern int FUN_00022b00(int,int);
extern unsigned int FUN_000232b0(void);
extern int FUN_000232c0(int);
extern int FUN_000232f8(void*);
extern int FUN_000777f0(int,int);
extern int FUN_0007d14a(int);
extern int FUN_00086be4(int,void*,int);

uint32_t set_device_sn_info(int param_1, uint32_t param_2, uint32_t *param_3, uint8_t *param_4){
    uint32_t uVar1;
    int iVar2;
    int8_t cVar3;
    uint8_t *puVar4;
    uint8_t auStack_9c[21];
    uint8_t auStack_87[115];
    DEBUG_PRINT(0x000a72e1);
    if (param_3==0 || param_4==0){ DEBUG_PRINT(0x000a72fc); uVar1=0xffffffff; }
    else if (*(volatile uint8_t*)0x20019ef3UL == 1){
        param_1 = param_1 + 4;
        puVar4 = (uint8_t*)*param_3;
        FUN_000232c0(param_1);
        FUN_000777f0(0x000a71cd, param_1);
        uVar1 = FUN_000167a8();
        FUN_00022b00(uVar1,0);
        if (2 < *(volatile int*)0x2000230cUL){
            if (*(volatile int*)0x20007554UL == 0){ uVar1=FUN_000232b0(); DEBUG_PRINT(0x000a71e0,0x000a76f2,uVar1); }
            else { uVar1=FUN_000232b0(); FUN_00019c70(0x000a71e0,0x000a76f2,uVar1); }
        }
        *puVar4=0x1c; puVar4[1]=1; puVar4[3]=1; cVar3=5; puVar4[2]=3; puVar4[4]=0; *param_4=5;
        do {
            iVar2 = FUN_000232f8(auStack_9c);
            if (iVar2==0 && (iVar2=FUN_00086be4(param_1, auStack_87, 0xe))==0){ DEBUG_PRINT(0x000a7209); goto done; }
            cVar3 = cVar3 - 1;
            FUN_0007d14a(100);
        } while (cVar3 != 0);
        puVar4[4]=1;
done:   uVar1=0;
    } else { DEBUG_PRINT(0x000a672f); uVar1=0xfffffffe; }
    return uVar1;
}

