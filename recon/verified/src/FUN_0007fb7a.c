/* Reconstructed FUN_0007fb7a @ 0x7fb7a  (parity: 300/300 trials, PROVEN) */
extern int FUN_0007f97c(unsigned,int,void*,int);
extern int FUN_0007f98a(unsigned,int,void*,int);
int FUN_0007fb7a(unsigned param_1, unsigned param_2){
    unsigned local_18 = param_1;
    unsigned local_14 = param_2;
    int iVar1 = FUN_0007f97c(param_1, 0x18, &local_14, 1);
    { unsigned char* p=(unsigned char*)&local_14; *p = (*p & 0xfd) | (((param_2>>7)&1)<<1); }
    int iVar2 = FUN_0007f98a(param_1, 0x18, &local_14, 1);
    iVar1 = iVar1 + iVar2;
    if (iVar1 == 0){
        iVar2 = FUN_0007f97c(param_1, 0x62, &local_18, 1);
        { unsigned char* p=(unsigned char*)&local_18; *p = (*p & 0xe7) | ((param_2 & 3)<<3); }
        iVar1 = FUN_0007f98a(param_1, 0x62, &local_18, 1);
        iVar1 = iVar1 + iVar2;
    }
    return iVar1;
}

