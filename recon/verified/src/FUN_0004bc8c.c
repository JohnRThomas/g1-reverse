...
extern int FUN_00086406(void); extern int FUN_00072908(void*,uint32_t,uint32_t);
...
void *FUN_0004bc8c(mpsc_buffer *b,uint32_t wlen,uint32_t to_lo,uint32_t to_hi){
 ...
 do { uint32_t key=checked_lock(b);
  ...
  else if((l.to_lo|l.to_hi)&&!FUN_00086406()){ checked_unlock(b,key); int e=FUN_00072908(b->sem,l.to_lo,l.to_hi); key=checked_lock(b); cont=(e==0); }
