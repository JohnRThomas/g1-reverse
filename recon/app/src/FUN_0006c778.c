...
extern float FUN_000759b8(float, int *);
extern void *FUN_00086c04(void *, const void *, unsigned);
...
#define lc3_sns_mpvq_offsets ((const int32_t (*)[11])0x00090128u)
#define lc3_sns_hfcb ((const float (*)[8])0x00090450u)
#define lc3_sns_lfcb ((const float (*)[8])0x00090850u)
#define lc3_band_lim ((const int (*)[LC3_NUM_SRATE][LC3_NUM_BANDS+1])0x00090c50u)
    ...
    const float *eb, bool att, float *scf)
{
    ...
    FUN_00086c04(e + 2*n2, eb + n2, (nb - n2) * sizeof(float));
    ...
    *(volatile int *)gain_idx = 0;
    ...
    for (int ic = 0; ic < 4; ic++) {
        const struct lc3_sns_vq_gains *cgains =
            (const struct lc3_sns_vq_gains *)0x000903e8u + ic;
