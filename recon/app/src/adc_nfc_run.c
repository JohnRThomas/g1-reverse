/* Reconstructed application adc_nfc_run @ 0x00024c14.
 * Raw/address back-map: FUN_00024c14 / 0x00024c14.
 */
#include <stdint.h>
#include <string.h>

struct device_raw {
    const char *name;
    const void *config;
    const void *api;
};

struct adc_api_raw {
    int (*channel_setup)(const struct device_raw *, const void *);
    int (*read)(const struct device_raw *, const void *);
    uint16_t scale;
};

struct adc_sequence_raw {
    uint32_t options;
    uint32_t channels;
    void *buffer;
    uint32_t buffer_size;
    uint8_t resolution;
    uint8_t oversampling;
    uint8_t calibrate;
    uint8_t reserved;
};

extern int FUN_0005f60c(int32_t whole, int32_t *fraction);
extern void FUN_00019c70(const void *, const void *, ...);
extern void FUN_0007dda4(const void *, const void *, ...);

uint32_t adc_nfc_run(void)
{
    int16_t sample;
    int32_t scaled;
    struct adc_sequence_raw sequence;
    memset(&sequence, 0, sizeof(sequence));
    sequence.channels = 8u;
    sequence.buffer = &sample;
    sequence.buffer_size = 2u;
    sequence.resolution = 12u;

    const struct device_raw *device =
        (const struct device_raw *)0x00087c20u;
    const struct adc_api_raw *api =
        (const struct adc_api_raw *)device->api;
    int result = api->read(device, &sequence);
    if (result < 0) {
        if (*(volatile int32_t *)0x2000230cu > 0) {
            if (*(volatile uint32_t *)0x20007554u == 0u) {
                FUN_0007dda4((const void *)0x0009f104u,
                             (const void *)0x0009f1c3u);
            } else {
                FUN_00019c70((const void *)0x0009f104u,
                             (const void *)0x0009f1c3u);
            }
        }
        return 0;
    }

    uint32_t value = (uint16_t)sample;
    scaled = (int32_t)(value * api->scale);
    result = FUN_0005f60c(0, &scaled);
    if (result == 0) {
        value = (uint32_t)scaled >> 12;
    } else if (result < 0) {
        if (*(volatile int32_t *)0x2000230cu > 0) {
            if (*(volatile uint32_t *)0x20007554u == 0u) {
                FUN_0007dda4((const void *)0x0009f120u,
                             (const void *)0x0009f1c3u);
            } else {
                FUN_00019c70((const void *)0x0009f120u,
                             (const void *)0x0009f1c3u);
            }
        }
        return 0;
    }

    return (value >= 1u && value <= 1199u) ? value : 0u;
}
