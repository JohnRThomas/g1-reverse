...
extern int  FUN_00082aee(uint32_t, uint32_t, void *, uint32_t,
                         uint32_t, uint32_t);
            ...
            const void *value, uint16_t id)
{
    ...
    FUN_00086c78(output.cleared, 0, 0x10);
    output.value = value;
    output.id = id;
...
void FUN_0005bd18(uint32_t context, int status, const uint8_t *data,
        ...
        while (stride <= remaining) {
            ...
            if (value.bytes[0] == 0) {
                ...
                *(uint16_t *)((uint8_t *)consumer + 8) = last_id;
                if (FUN_00082aee(context, UINT32_C(0x0005c005), consumer,
            ...
            if (consumer[0] == 0 || FUN_00080d3e(value.bytes) == 0) {
