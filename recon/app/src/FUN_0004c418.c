...
extern void FUN_0004c278(uint32_t id, uint32_t channel, uint32_t priority,
                        uint32_t zero);
...
void FUN_0004c418(void *object, uint32_t context, uint32_t descriptor)
{
    ...
    if (prepare == 0 || prepare(object, context, 0) != 0) {
        FUN_0007e2fa(0x00099cbdu, 0x000a7a10u, 0x000f0a90u, 0xcbu);
        FUN_0007e2ec(0x000f0a90u, 0xcbu);
    }

    uint32_t channel = (descriptor >> 8) & 0xffu;
    int32_t timing = FUN_000635d8(channel);
    uint32_t id = descriptor & 0xffu;
    FUN_0004c278(id, channel, descriptor >> 24, 0);
    uint32_t state = FUN_00063570(channel);
    uint32_t slot = (descriptor >> 16) & 0xffu;

    if (slot >= 16) {
        FUN_0007e2fa(0x00099cbdu, 0x000f0a5du, 0x000f0a2bu, 0x1e1u);
        FUN_0007e2ec(0x000f0a2bu, 0x1e1u);
    }
