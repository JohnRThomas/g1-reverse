...
extern int FUN_00068b14(int sr);
extern int FUN_0006b36c(bool pitch);
extern int FUN_0006efd8(void);
extern int FUN_000709d8(const void *tns);
extern void FUN_0006f164(int dt,int sr,int gain,float *x,uint16_t *xq,int *nq);
extern int FUN_0006f28c(int dt,int sr,int nbytes,const uint16_t *xq,int *nq,
                       int budget,bool *lsb_mode);
extern float FUN_00075d5c(float x);
...
{
    const uint16_t *t=(const uint16_t *)0x0008d350u;
                         ...
                         int goff,bool *reset)
{
    ...
    if(xmax==0) gmin=-goff;
        ...
        float z=FUN_00075d5c(__builtin_sqrtf(xmax)*(1.0f/(32768.0f-0.375f)));
...
void FUN_0006f4a0(int dt,int sr,int nbytes,bool pitch,const void *tns,
                  ...
                  lc3_spec_side_t *side)
{
    ...
    int budget=8*nbytes-ac-FUN_00068b14(sr)-FUN_0006b36c(pitch)-FUN_0006efd8()-
      FUN_000709d8(tns)-8-3;
    ...
    FUN_0006f164(dt,sr,gint,x,xq,&side->nq);
    int nbits=FUN_0006f28c(dt,sr,nbytes,xq,&side->nq,0,0);
    spec->nbits_off=reset?0:off; spec->nbits_spare=reset?0:budget-nbits;
    int adj=adjust_gain(sr,gint+goff,nbits,budget);
    if(adj)FUN_0006f164(dt,sr,adj,x,xq,&side->nq);
