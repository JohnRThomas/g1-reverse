/* Reconstructed FUN_0005c310 @ 0x0005c310 (gatt_prepare_write_rsp).
 * Raw/address backmap: FUN_0005c310 @ 0x0005c310, extent 0x0000009c.
 */
#include <stdint.h>
extern void FUN_00082a42(uint32_t, uint32_t, const void *); /* logger */
extern int FUN_00082aee(void *, void *, void *, void *, uint32_t, uint32_t);
extern int FUN_00086be4(const void *, const void *, uint32_t); /* memcmp */
extern int FUN_0005c22c(void *, void *); /* bt_gatt_write */

struct write_params_recovered {
    void (*callback)(void *, uint8_t, void *);
    uint16_t handle;
    uint16_t offset;
    const uint8_t *data;
    uint16_t length;
};

void FUN_0005c310(void *connection, const uint8_t *response,
                  uint16_t response_length, struct write_params_recovered *params)
{
    uint16_t payload_length = (uint16_t)(response_length - 4U);
    if (payload_length > params->length) {
        uint32_t package[2] = { 2U, 0x000f4cddU };
        FUN_00082a42(0x00088128U, 0x1040U, package);
        if (FUN_00082aee(connection, (void *)0x00082603U, params,
                         (void *)0x0008278bU, 0x18U, 1U) != 0)
            params->callback(connection, 0x0eU, params);
        return;
    }
    int data_valid = FUN_00086be4(params->data, response + 4,
                                  payload_length) == 0;
    if (params->offset != *(const uint16_t *)(response + 2) || !data_valid) {
        uint32_t package[2] = { 2U, 0x000f4cffU };
        FUN_00082a42(0x00088128U, 0x1040U, package);
        if (FUN_00082aee(connection, (void *)0x00082603U, params,
                         (void *)0x0008278bU, 0x18U, 1U) != 0)
            params->callback(connection, 0x0eU, params);
        return;
    }
    params->offset = (uint16_t)(params->offset + payload_length);
    params->data += payload_length;
    params->length = (uint16_t)(params->length - payload_length);
    int error;
    if (params->length == 0U)
        error = FUN_00082aee(connection, (void *)0x00082603U, params,
                             (void *)0x0008279dU, 0x18U, 1U);
    else
        error = FUN_0005c22c(connection, params);
    if (error != 0)
        params->callback(connection, 0x0eU, params);
}
