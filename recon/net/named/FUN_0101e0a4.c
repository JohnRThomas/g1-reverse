...
#include <stdint.h>
extern void sdc_assertion_fail(unsigned reason,unsigned line);
...
{
    if(context==0) sdc_assertion_fail(0x1a,0xb1);
    ...
    if(value==0) context[0xc6]=0x2b;
        ...
        if(entry!=0){
            ...
            if(target==0) sdc_assertion_fail(0x1a,0xd3);
            FUN_0101df84(target,entry);
        }
    }
    if((int8_t)context[0x104]<0) sdc_assertion_fail(0x1a,0xd8);
    return 0;
}
