 ...
 *   thunk_FUN_01025034                       <= FUN_01022f08 @ 0x01022f08
...
extern uint64_t thunk_FUN_01025034(void);
extern uint32_t FUN_01012f18(void *, uint32_t, uint32_t, uint32_t, uint16_t *);
...
extern int32_t FUN_0100f0fc(void *);
...
{
    uint64_t elapsed = thunk_FUN_01025034() - U64(context, 0x350);
    ...
    while ((int16_t)(limit - next) >= 0) {
        uint16_t consumed = 0;
        uint32_t result = FUN_01012f18(context, next, 0, 0, &consumed);
        if (result == 0) {
            int16_t distance;
            uint16_t period;
...
static __attribute__((always_inline)) inline void finish_interval(void *context)
{
    ...
    if (U32(context, 0x324) - 0x92u + budget < U32(context, 0x14))
        ...
        available = 0;
    if (U8((void *)0x21000f54u, 8) == 0 && U32(context, 0x30) <= available)
        available = U32(context, 0x30);

    fixed = FUN_0100f0fc((uint8_t *)context + 0x30);
