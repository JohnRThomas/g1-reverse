...
#define CURRENT_OWNER ((volatile uint32_t *)0x21004b30u)
#define KERNEL_LOCK ((uint32_t *)0x21004b48u)

int32_t FUN_0103657c(uint32_t *object)
{
    ...
    if (ipsr != 0) {
        FUN_01039bbe(LOG_MODULE, LOG_STRING, 0xcdu);
    ...
    count = object[3];
    if (count == 0) {
        FUN_01039bbe(LOG_MODULE, LOG_STRING, 0xe5u);
    ...
    if (FUN_0103610c(KERNEL_LOCK) == 0) {
        FUN_01039bbe(LOG_MODULE, ASSERT_STRING, 0x72u);
    ...
    if (FUN_01036128(KERNEL_LOCK) == 0) {
        FUN_01039bbe(LOG_MODULE, ASSERT_STRING, 0xf0u);
