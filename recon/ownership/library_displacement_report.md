...
> ### Batch 1 APPLIED — 2026-07-25 (P4 iteration 8 Step B)
>
> **Status: applied, clean, no regression, nothing reverted.** Full record:
> `recon/emulator/reports/our_boot_bringup.md` §Iteration 8 Step B.
>
> * Membership was re-derived from `library_displacement_candidates.json` with
>   this section's own predicate (`core == app && confidence == high &&
>   upstream_object_in_link && referenced_by_linker_artifact == [] &&
>   callers.even_owned == 0 && component in {libc_newlib_nano, zcbor,
>   zephyr_lib_os, zephyr_arch, zephyr_soc, zephyr_libc}`) and reproduces the
>   documented **52 rows / 8 186 B** exactly (`libc_newlib_nano` 25, `zcbor` 9,
>   `zephyr_lib_os` 7, `zephyr_arch` 7, `zephyr_soc` 2, `zephyr_libc` 2).
> * All 52 `exclude_reconstruction: true` rows added to
>   `recon/ownership/adoption_manifest.json` (`cores.app.entries`), each with the
>   candidate's own evidence records plus a `library_displacement_candidate`
>   record carrying its upstream linkage, caller counts and boot-path flags.
>   App manifest **571 → 623** entries, **557 → 609** exclusions.
>   `recon/generated/app_retained_sources.cmake` regenerated by
>   `tools/gen_retained_sources.py` only (retained 1707 → **1655**, matched
>   sources 412 → **464**); `--check` passes. Confirmed in the build tree: all
>   52 reconstruction TUs compiled in the pre-batch build, **none** in the
>   post-batch build.
> * **Caller wiring was required, as §5 predicted.** Twelve of the displaced
>   identities (`Balloc`, `Bfree`, `multadd`, `pow5mult`, `bigint_mult`,
>   `lshift`, `bigint_diff`, `clz32`, `bignum_compare_abs`, `d2b_decompose`,
>   `sbrk_impl`, `smakebuf_r`) are still called *by name* from recovered newlib
>   TUs outside the batch (`dtoa_r.c`, `i2b.c`, `bignum_div_trial_subtract.c`,
>   `stdio_fclose.c`, `_sbrk_r.c`), so the first link failed with undefined
>   references. New bridge TU
>   **`recon/application/app/src/g1_newlib_mprec_bridges.c`** (the Batch 0
>   pattern) forwards each historical identity onto the genuine newlib entry
>   point (`_Balloc`, `_Bfree`, `__multadd`, `__pow5mult`, `__multiply`,
>   `__lshift`, `__mdiff`, `__hi0bits`, `__mcmp`, `__d2b`, `_sbrk`,
>   `__smakebuf_r`). Argument lists come from the *proven* reconstruction
>   prototypes; trailing Ghidra-artifact parameters are accepted and dropped.
>   GCC emits a 4-byte `b.w` per bridge. **No recovered caller source was
>   edited.**
> * **Measured:** rebuild `/private/tmp/g1-i8b-app` exit 0, **0 undefined,
>   0 duplicate**; FLASH 629 932 → **627 680 B (−2 252 B, 63.88 %)**; RAM
>   unchanged at 75 757 B, so no RAM-pin re-shuffle. The delta is smaller than
>   the 8 186 B of displaced extent because most of those bodies were already
>   `--gc-sections` casualties and several upstream members are newly pulled in
>   (notably `_sbrk`, which was not previously in the link at all).
> * **Boot: no regression.** 5 041 553 instructions / **672** unique at 0.15 s
>   and 5 149 911 / **681** at 3.0 s, versus 5 041 477 / 671 and 5 151 848 / 680
>   before the batch. The trace diff is exactly **one new function** — `sbrk`,
>   the genuine Zephyr libc hook now reached through the displaced `sbrk_impl` —
>   and **zero** functions lost; every marker shifts by the same +76
>   instructions the bridge thunks cost, in the same order.
> * **Rows reverted: none.**
> * **Not yet done:** the 52 reconstruction TUs remain on disk in
>   `recon/symbolized/app/` (the manifest is the only exclusion authority, per
>   policy). The five out-of-batch newlib callers still reference the historical
>   names through the bridge rather than calling `__multadd` &c. directly; that
>   is a source-edit cleanup for G3, not a displacement blocker.
...
> ### Batch 2 APPLIED — 2026-07-25 (P4 iteration 9 Step B)
>
> Two sub-batches were attempted against the iteration-9 Step-A app baseline
> (`/private/tmp/g1-i9e-app`: 5 647 329 instructions / **762** unique app
> functions before the boot's own divergence, FLASH 627 680 B, RAM 75 757 B).
> Eligibility was re-derived from `library_displacement_candidates.json` with
> the §4 predicate `core == app && confidence == high &&
> upstream_object_in_link && referenced_by_linker_artifact == [] &&
> callers.even_owned == 0`, minus every row already excluded by Batch 0/1,
> every `keep_reconstructed` row and every
> `conflicts_with_manifest_retain_row` row: **211 eligible app rows**, of which
> 102 are `zephyr_bt_host` (deferred to Batch 6, per §4) and **109 are
> non-Bluetooth**. All 33 net-core candidates remain deferred (§7 FLASH risk),
> which iteration 9 independently confirmed: the net image is now at
> **99.02 %** of its 226 KB FLASH region.
>
> #### B2a — `zephyr_drivers` (39 rows, 4 536 B): **TRIED, REGRESSED, FULLY REVERTED**
>
> * The first link exposed **five** undefined references from recovered TUs
>   outside the sub-batch. Two have a public upstream symbol and were bridged in
>   a new `recon/application/app/src/g1_zephyr_driver_bridges.c`:
>   `dev_write_reg4 → mfd_npm1300_reg_write2` and
>   `rtc_read_extended_cycle_count → z_nrf_rtc_timer_read`. The other three map
>   to a `static inline` or a `.constprop` clone with **no linkable symbol** —
>   `dev_ctrl_write1 → i2c_write_dt`,
>   `flash_page_index_lookup → linear_range_get_win_index`,
>   `layout_select_region → linear_range_group_get_win_index.constprop.0` —
>   so those three rows were dropped from the sub-batch (36 rows remained).
> * **Measured:** rebuild `/private/tmp/g1-i9f-app` exit 0, 0 undefined,
>   0 duplicate; FLASH 627 680 → 627 584 B (−96 B); RAM unchanged.
> * **Boot: REGRESSION.** 5 654 326 instructions / **690** unique versus the
>   baseline's 5 647 329 / **762**. The run dies on
>   `ASSERTION FAIL [... (p_instance->drv_inst_idx) == NRFX_TWIM1_INST_IDX ...]
>   @ modules/hal/nordic/nrfx/drivers/src/nrfx_twim.c:593` inside the displaced
>   TWIM/I2C path, immediately after `power_for_panel → dev_write_reg3 →
>   dev_ctrl_write1`, and loses 76 functions including `power_for_imu_and_mic`,
>   `lsm6dso_init_chip`, the audio codec, the fuel gauge, the settings/NVS
>   writes and the BLE work thread. The displaced upstream body evidently
>   drives a different instance descriptor than the recovered one.
> * **Reverted in full** (36 manifest rows, the bridge TU and its CMake entry).
>   This is direct empirical support for §4's ordering, which already placed
>   `zephyr_drivers` in Batch 3 with a per-upstream-unit application order —
>   **re-apply one unit at a time** (`nrf_rtc_timer.c` first, `mfd_npm1300.c`
>   and `nrfx_twim.c` last) rather than as one sweep.
>
> #### B2b — `zephyr_kernel` (16 rows, ~2.4 KB): **APPLIED, CLEAN**
>
> * 18 candidates matched; **2 reverted** because their upstream body is
>   file-static in `zephyr/kernel/sched.c` with no linkable symbol —
>   `z_tick_sleep` (0x000746fc) and `unready_thread` (0x00073e88). Both were
>   discovered by an undefined reference from an out-of-batch recovered caller.
> * **16 rows applied** (app manifest 623 → 639 entries, 609 → 625 exclusions;
>   `app_retained_sources.cmake` regenerated by `tools/gen_retained_sources.py`
>   only, retained 1655 → 1637, matched sources 464 → 482, `--check` clean):
>
>   | VA | recovered name | upstream owner |
>   |---|---|---|
>   | 0x000736ec | `sched_thread_should_preempt` | `sliceable` |
>   | 0x00073788 | `sched_switch_handle_timeout` | `z_reset_time_slice` |
>   | 0x000738d4 | `z_ready_thread_locked` | `z_ready_thread` |
>   | 0x00073960 | `abort_thread_timeout_locked` | `z_unpend_thread_no_timeout` |
>   | 0x00073d1c | `sched_requeue_by_prio` | `move_thread_to_end_of_prio_q` |
>   | 0x00073ec0 | `add_thread_to_wait_queue` | `add_to_waitq_locked` |
>   | 0x00074060 | `z_sched_set_prio_and_requeue` | `z_set_prio` |
>   | 0x000741a4 | `z_thread_suspend` | `z_impl_k_thread_suspend` |
>   | 0x000742b4 | `unpend_thread` | `z_unpend_thread` |
>   | 0x0007440c | `z_timer_stop_unpend` | `z_unpend1_no_timeout` |
>   | 0x00074b7c | `next_timeout` | `next_timeout` |
>   | 0x00074bbc | `remove_timeout` | `remove_timeout` |
>   | 0x000751d0 | `register_events` | `register_events` |
>   | 0x000753ec | `clear_event_registrations` | `clear_event_registrations` |
>   | 0x000757b0 | `poll_signal_event_locked` | `z_impl_k_poll_signal_raise` |
>   | 0x000867da | `signal_poll_event` | `signal_poll_event` |
>
> * **Callers wired to the real API.** Five historical identities are still
>   called by name from recovered TUs outside the sub-batch and forward onto the
>   public Zephyr entry points in the new
>   **`recon/application/app/src/g1_kernel_sched_bridges.c`** (the Batch 0/1
>   bridge pattern): `sched_switch_handle_timeout → z_reset_time_slice`,
>   `z_ready_thread_locked → z_ready_thread`,
>   `z_sched_set_prio_and_requeue → z_set_prio`,
>   `z_thread_suspend → z_impl_k_thread_suspend`,
>   `poll_signal_event_locked → z_impl_k_poll_signal_raise`. Argument lists come
>   from the proven reconstruction prototypes; the trailing Ghidra-artifact
>   parameters the shipped bodies never read are accepted and dropped. **No
>   recovered caller source was edited.**
> * **Measured:** rebuild `/private/tmp/g1-i9g-app` exit 0, **0 undefined,
>   0 duplicate**; FLASH 627 680 → **626 416 B (−1 264 B, 63.76 %)**; RAM
>   unchanged at 75 757 B, so **no RAM-pin re-shuffle**.
> * **Boot: no regression.** 5 612 329 instructions / **760** unique versus the
>   baseline's 5 647 329 / **762**, ending at the *identical* divergence
>   (`display_dev_reg_config → wait_for_event → z_tick_sleep → … → z_arm_pendsv`
>   restoring a thread whose saved PC is 0). The two absent names are
>   `sched_thread_should_preempt` — itself one of the 16 displaced rows — and
>   `sched_ready_queue_insert` (0x00073840), whose only caller was the displaced
>   `z_ready_thread_locked` and which is therefore now garbage-collected. Both
>   are direct consequences of the displacement; **zero behaviour was lost.**
> * **Rows reverted: 2** (`z_tick_sleep`, `unready_thread` — see above).
>
> #### Cumulative totals after Batch 2
>
> | | rows displaced | FLASH delta |
> |---|---:|---:|
> | Batch 0 (`gpio.h` inlines, iteration 7) | 4 | (bundled with iteration 7) |
> | Batch 1 (libc/zcbor/lib-os/arch leaves, iteration 8) | 52 | −2 252 B |
> | **Batch 2b (`zephyr_kernel`, iteration 9)** | **16** | **−1 264 B** |
> | Batch 2a (`zephyr_drivers`, iteration 9) | 0 (36 tried, all reverted) | — |
> | **cumulative** | **72** | **−3 516 B** |
>
> App manifest: **639** entries / **625** exclusions. Not yet started:
> `hal_nordic_nrfx` (27 eligible), `nrf_subsys` (10), `zephyr_logging` (3),
> `hal_st` (3), `zephyr_ipc` (3), `zephyr_mcumgr` (2), `zephyr_settings` (1),
> `other` (3), `zephyr_drivers` (39, unit-by-unit only), `zephyr_bt_host` (102)
> and all 33 net-core rows.
...
## Batch 3 applied — 2026-07-25 (P4 iteration 10 Step B)

**Status: 4 units applied clean, 1 unit reverted. FLASH delta 0 B. Boot
no-regression proven by image identity.** Full context:
`recon/emulator/reports/our_boot_bringup.md` §Iteration 10 Step B.

Per the Batch-2a lesson, units were applied **one upstream unit at a time**,
each with its own `tools/gen_retained_sources.py` run, clean build and
undefined/duplicate check.

| # | upstream unit | rows | ledger extent | link | FLASH delta | image vs previous | outcome |
|---|---|---:|---:|---|---:|---|---|
| 1 | `modules/hal/nordic/nrfx/drivers/src/nrfx_gpiote.c` | 10 | 1300 B | 0 undef / 0 dup | **0 B** | byte-identical | **APPLIED** |
| 2 | `modules/hal/nordic/nrfx/drivers/src/nrfx_spim.c` | 6 | 1062 B | 0 undef / 0 dup | **0 B** | byte-identical | **APPLIED** |
| 3 | `zephyr/drivers/flash/soc_flash_nrf.c` | 2 | 324 B | 0 undef / 0 dup | **0 B** | byte-identical | **APPLIED** |
| 4 | `zephyr/drivers/regulator/regulator_common.c` | 3 | 266 B | 0 undef / 0 dup | **0 B** | byte-identical | **APPLIED** |
| 5 | `modules/hal/st/.../lsm6dso_STdC/driver/lsm6dso_reg.c` | 3 | 190 B | **link FAILED** | — | — | **REVERTED** |

App manifest: **639 → 660 entries, 625 → 646 exclusions.**
`recon/generated/app_retained_sources.cmake` regenerated by the sanctioned
generator only; `--check` passes. No bridge TU was needed for units 1–4, no
recovered caller source was edited, no `tools/` change, nothing committed.

### Why unit 5 was reverted

Excluding the three `lsm6dso_reg.c` rows left two Even-owned callers with
undefined references:

```
recon/symbolized/app/audio_codec_set_gain_ch0.c:24: undefined reference to `audio_codec_read_gain_step_mode'
recon/symbolized/app/audio_codec_set_gain_ch1.c:24: undefined reference to `audio_codec_read_gain_step_mode'
```

The candidate record maps `audio_codec_read_gain_step_mode` onto an
`lsm6dso_reg.c` register accessor. An **audio-codec** gain read is not plausibly
the same identity as an **LSM6DSO** register helper, so this is a false byte
match (the two are both trivial `read register, shift, mask` bodies), and the
right response is to reject the row rather than bridge it. The three rows were
removed from the manifest, the list regenerated and the build re-verified clean
(image again byte-identical).

### The important measured finding: most remaining candidates are already gc'd

Every applied unit produced a **byte-identical linked image**
(`md5 f10f2fbd3a53e29c1d3a74c9c1113080` for the Step A build and for all four
Batch-3 builds). The reason is that the cohesive app build now runs with
`G1_INTEGRATION_PROBE_RETAIN_ALL=OFF`, i.e. `--gc-sections`: a reconstruction
with no live caller is **already absent from the image**, so excluding it in the
manifest changes only the ledger. Batches 0–2b did save FLASH (−3,516 B) because
those rows *were* the live owners.

A correct way to find rows that can still change the image is to parse only the
**kept**-section part of `zephyr.map` (everything from `Linker script and memory
map` onwards — the discarded-sections block at the top must be skipped, which is
the mistake that made units 3 and 4 look promising) and keep candidates whose
`.text.<current_symbol>` section is contributed by `app/libapp.a`. Applied to
`/private/tmp/g1-i10v-app`, the **entire remaining non-BT headroom is 15 rows /
1068 B**:

| upstream unit | live rows | bytes | note |
|---|---:|---:|---|
| `nrf/subsys/bluetooth/gatt_dm.c` | 8 | 580 | Bluetooth-adjacent; excluded by the standing BT policy |
| `zephyr/drivers/sensor/npm1300_charger/npm1300_charger.c` | 2 | 152 | `flash_page_index_lookup` / `layout_select_region` — already known from B2a to have **no linkable upstream symbol** (`linear_range_get_win_index` is `static inline` / `.constprop`) |
| `zephyr/subsys/logging/log_core.c` | 1 | 108 | `log_dropped` |
| `zephyr/drivers/mfd/mfd_npm1300.c` | 2 | 80 | `dev_ctrl_write1` is Zephyr's header `static inline` `i2c_write_dt` — no external symbol (dropped in B2a); it is also the function iteration 10 §A.3 just defect-fixed |
| `zephyr/drivers/timer/nrf_rtc_timer.c` | 1 | 76 | `rtc_read_extended_cycle_count` — needs the B2a bridge to `z_nrf_rtc_timer_read` |
| `modules/hal/st/.../lsm6dso_reg.c` | 1 | 72 | the rejected identity above |

So goal-G2 progress against the *live image* is essentially complete for non-BT
code: what is left is the 217 BT-host rows (still excluded, to be done unit by
unit once the BT stack actually starts), plus the handful above that need
bridges or that are false matches. The 93 applied rows remain valuable as an
ownership statement even where they cost 0 bytes: they record that the
reconstruction is not the owner, so a future `RETAIN_ALL=ON` or gc-off build
cannot silently resurrect a duplicate body.

### Cumulative totals after Batch 3

| batch | rows | FLASH delta | status |
|---|---:|---:|---|
| Batch 0 (`gpio.h` inlines, iteration 7) | 4 | (bundled with iteration 7) | applied |
| Batch 1 (libc/zcbor/lib-os/arch leaves, iteration 8) | 52 | −2,252 B | applied |
| Batch 2a (`zephyr_drivers`, 39 rows at once, iteration 9) | 0 (36 tried) | — | **reverted** (boot regression, `nrfx_twim.c:593`) |
| Batch 2b (`zephyr_kernel` sched/poll, iteration 9) | 16 | −1,264 B | applied |
| **Batch 3 — 4 upstream units (iteration 10)** | **21** | **0 B** | **applied** |
| Batch 3 — `lsm6dso_reg.c` (iteration 10) | 0 (3 tried) | — | **reverted** (undefined refs, false identity) |
| **cumulative** | **93** | **−3,516 B** | app manifest 660 entries / 646 exclusions |

Net-core rows: still **zero** applied (net is at 99.02 % FLASH and cannot absorb
growth). `keep_reconstructed` and `conflicts_with_manifest_retain_row` rows
remain untouched.
