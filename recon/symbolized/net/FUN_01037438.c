...
void z_reschedule_unlocked(void)
{
    ...
    cached = *(uint8_t **)((unsigned long)&g_zephyr_sched_swap_tmp) /*=0x21004b5c*/;

    if (cached == current) {
        swap_current(current);
    } else {
        *(uint32_t *)((unsigned long)&g_zephyr_sched_swap_tmp) /*=0x21004b5c*/ = 0;
        uint8_t cpu = *(uint8_t *)((unsigned long)&g_21004b38) /*=0x21004b38*/;
