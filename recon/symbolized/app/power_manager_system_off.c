...
void power_manager_system_off(void){
  ...
  if(r5 != 0){
    printk(((unsigned long)"ASSERTION FAIL [%s] @ %s:%d\n") /*=0x99cbd*/,((unsigned long)"power_state == POWER_STATE_IDLE") /*=0xf6749*/,((unsigned long)&rodata_f6716) /*=0xf6716*/,0xab);
    assert_post_action(((unsigned long)&rodata_f6716) /*=0xf6716*/,0xab);
  }
  L:
  iVar1 = ancs_notify_table_is_idle(r5);
  if(iVar1==0){
    printk(((unsigned long)"ASSERTION FAIL [%s] @ %s:%d\n") /*=0x99cbd*/,((unsigned long)"check_if_power_state_allowed(POWER_MANAGER_LEVEL_SUSPENDED)") /*=0xf6769*/,((unsigned long)&rodata_f6716) /*=0xf6716*/,0xac);
