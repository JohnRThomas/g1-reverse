...
int FUN_00054d18(int index, int handle, unsigned int value)
{
    ...
    if (handle != 0 && bt_addr_le_eq(handle, ((unsigned long)&rodata_f2b3a) /*=0xf2b3a*/) == 0) {
        FUN_00080fc4(((unsigned long)&g_ble_dev_state) /*=0x20002000*/ + index * 7, handle);
    } else {
        do {
        ...
        result = FUN_00080fc4(((unsigned long)&g_ble_dev_state) /*=0x20002000*/ + index * 7,
                              (int)&operation);
        if (handle != 0)
            FUN_00080fc4(handle, result);
    }

    result = FUN_00080fb4(((unsigned long)&g_ble_dev_ncmd_sem) /*=0x200020d4*/);
