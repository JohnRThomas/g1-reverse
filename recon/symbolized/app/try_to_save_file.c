...
void try_to_save_file(int context)
{
    ...
    if (packet_length < 0x15 && packet[0] == 0x0d && packet[1] == 0x0e) {
        ...
        if (BUFFERED_LENGTH < 0x1000) {
            ...
            DEBUG_PRINT(((unsigned long)&rodata_9e88b) /*=0x9e88b*/, ((unsigned long)&rodata_9e8e5) /*=0x9e8e5*/, checksum);
            return;
        }
        goto save_block;
    }
...
save_block:
    ...
    if (LOG_LEVEL > 0) {
        if (LOG_SINK == 0)
            DEBUG_PRINT(((unsigned long)&rodata_9e873) /*=0x9e873*/, ((unsigned long)&rodata_9e8e5) /*=0x9e8e5*/, checksum);
        else
            debug_print();
    }
    FILE_ADDRESS += 0x1000;
