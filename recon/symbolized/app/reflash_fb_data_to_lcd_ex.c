...
/* named: reflash_fb_data_to_lcd_ex */
/* globals referenced:
//   0x2000230c  g_log_level                  
//   0x20007554  g_log_use_alt_sink           
//   0x2000a060  g_projector_bus_lock         
*/
/* Reconstructed reflash_fb_data_to_lcd_ex @ 0x473c8  (parity: 300/300 trials, PROVEN) */
#include <stdint.h>
...
unsigned reflash_fb_data_to_lcd_ex(int param_1,int param_2,int param_3,int param_4,int param_5,int param_6,int param_7)
{
    ...
    if(*(int*)(iVar2+0x35c) == 0){
        if(0 < *(volatile int*)((uintptr_t)&g_log_level) /*=0x2000230c*/){
            if(*(volatile int*)((uintptr_t)&g_log_use_alt_sink) /*=0x20007554*/ == 0){ DEBUG_PRINT("%s(): c->init_done = 0,init failed, can't write data!\n" /*=0xaa891*/,"_reflash_fb_data_to_lcd_ex" /*=0xd7426*/); }
            else { debug_print(0); }
        }
        uVar3 = 0xffffffff;
    } else {
        ...
        for(; param_4 <= param_6; param_4 = param_4 + 1){
            ...
            uVar6 = ((uintptr_t)&rodata_7fc00) /*=0x7fc00*/ & (unsigned)((param_4+param_2)*0x400);
            ...
            *(unsigned char*)(iVar7+iVar9) = 0;
            k_mutex_lock(((uintptr_t)&g_projector_bus_lock) /*=0x2000a060*/, uVar3, 0xffffffff, 0xffffffff);
            ...
            if(iVar4 != 0 && 0 < *(volatile int*)((uintptr_t)&g_log_level) /*=0x2000230c*/){
                if(*(volatile int*)((uintptr_t)&g_log_use_alt_sink) /*=0x20007554*/ == 0){
                    uVar3 = (unsigned)((u64)DEBUG_PRINT("%s(): spi transfer error....\n" /*=0xd7383*/,"_reflash_fb_data_to_lcd_ex" /*=0xd7426*/) >> 32);
                } else {
                    uVar3 = (unsigned)((u64)debug_print(0) >> 32);
                }
            }
