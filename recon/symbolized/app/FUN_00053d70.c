...
int FUN_00053d70(uint32_t opcode, void *command, void **result)
{
    ...
    *(void **)(((unsigned long)&bt_hci_cmd_data) /*=0x2000abf4*/ + (uint32_t)slot * 12u + 8u) = completion;
    uintptr_t token = net_buf_ref(command, (uint32_t)(indexed >> 32), 1, 12);
    uint64_t inherited = net_buf_destroy_default(((unsigned long)&hci_cmd_pool) /*=0x2000214c*/, token, 1, 12);
    ...
    struct log_record4 record = {4, ((unsigned long)&rodata_f304a) /*=0xf304a*/, opcode, status};
    FUN_00080ea2(((unsigned long)&rodata_88138) /*=0x88138*/, 0x2080u, &record);
