...
void ancs_security_changed(int param_1, int param_2, int param_3)
{
  ...
  volatile int *piVar2 = (volatile int*)((uintptr_t)&g_ancs_security_fail_cnt) /*=0x20006a34*/;
  ...
  if (param_3 == 0) {
    if (2 < iVar5) {
      if (*E == 0) DEBUG_PRINT("%s(): %s level %u\n\n" /*=0x9a52b*/,"ancs_security_changed" /*=0x9b229*/,(int)stack38,param_2);
      else debug_print("%s(): %s level %u\n\n" /*=0x9a52b*/,"ancs_security_changed" /*=0x9b229*/,(int)stack38,param_2);
    }
    unsigned int uVar4 = (unsigned int)bt_conn_get_security(param_1);
    if (1 < uVar4) {
      *(volatile int*)((uintptr_t)&g_gatt_discovery_flags) /*=0x20006ab4*/ = 0;
    ...
    *piVar2 = 0;
    ...
    if (2 < iVar5) {
      ...
      debug_print("%s(): Security failed: %s level %u err %d\n\n" /*=0x9a53f*/,"ancs_security_changed" /*=0x9b229*/,(int)stack38,param_2,param_3);
    }
    while (1) {
      iVar5 = *piVar2;
      *piVar2 = iVar5 + 1;
      if (iVar5 + 1 < 10) break;
      if (0 < *piVar1) {
        if (*E == 0) DEBUG_PRINT("%s(): reboot because Security failed exceed 10 times, so reboot\r\n\n" /*=0x9a56b*/,"ancs_security_changed" /*=0x9b229*/);
        else debug_print(0);
      }
      FUN_0007c0a8(500);
      sys_reboot(1);
    LAB_185da:
      DEBUG_PRINT("%s(): Security failed: %s level %u err %d\n\n" /*=0x9a53f*/,"ancs_security_changed" /*=0x9b229*/,(int)stack38,param_2);
    }
