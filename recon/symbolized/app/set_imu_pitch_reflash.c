...
unsigned set_imu_pitch_reflash(void)
{
    ...
    if (*(volatile uint8_t*)(iVar1 + 0xf0) < 2) {
        if (*(volatile uint8_t*)((uintptr_t)&g_imu_pitch_last_state) /*=0x2001cf91*/ != *(volatile uint8_t*)(iVar1 + 0xf0)) {
            gui_screen_clear();
            *(volatile uint8_t*)((uintptr_t)&g_imu_pitch_last_state) /*=0x2001cf91*/ = *(volatile uint8_t*)(iVar1 + 0xf0);
        }
        ...
        if (0 < *(volatile int*)((uintptr_t)&g_log_level) /*=0x2000230c*/) {
            if (*(volatile int*)((uintptr_t)&g_log_use_alt_sink) /*=0x20007554*/ == 0) DEBUG_PRINT();
            else debug_print();
        }
        return 0xffffffff;
    }
