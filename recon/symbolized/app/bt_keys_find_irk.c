...
uint8_t* bt_keys_find_irk(uint param_1, uint8_t* param_2, uint param_3, uint param_4){
    ...
    if(FUN_00086be4(param_2+1,((unsigned long)&bt_keys_pool) /*=0x2000af4c*/+0x96,6)!=0) goto L8a2;
    goto L8fe;
L894:
...
L8d4:
    if(FUN_00086be4(param_2+1,((unsigned long)&bt_keys_pool) /*=0x2000af4c*/+0x3a,6)==0) return (uint8_t*)((int)r4+0);
    if(!(((uint)*(volatile uint16_t*)(r4+0x6a)<<0x1e)&0x80000000u)) goto L904;
L8ea:
    if((uint)*(volatile uint8_t*)(r4+0x5c) != param_1) goto L904;
    if(FUN_00086be4(param_2+1,((unsigned long)&bt_keys_pool_1_addr) /*=0x2000afe2*/,6)!=0) goto L904;
L8fe:
