#include "g1_app_symbols.h"
/* named: ble_rng_get_random_bytes */
/* Reconstructed ble_rng_get_random_bytes @ 0x55c64  (parity: 300/300 trials, PROVEN) */

extern int FUN_0004d944(int,int,void*,int);
extern int bt_hci_le_rand(void*,int);
extern int ble_rng_seed_from_uptime(void);
extern int tc_hmac_prng_init(int,void*,int);

int ble_rng_get_random_bytes(void)
{
    unsigned char buf[24];
    int iVar1 = bt_hci_le_rand(buf, 8);
    if (iVar1 == 0) {
        iVar1 = tc_hmac_prng_init(((uintptr_t)&g_hmac_prng_ctx) /*=0x20006000*/, buf, 8);
        if (iVar1 == 0) {
            struct { int f0; int f4; } s;
            s.f0 = 2;
            s.f4 = "Failed to initialize PRNG" /*=0xf3966*/;
            iVar1 = -5;
            FUN_0004d944(((uintptr_t)&tbl_880d8) /*=0x88148*/, 0x1040, &s, 0);
        } else {
            iVar1 = ble_rng_seed_from_uptime();
        }
    }
    return iVar1;
}

