#include "g1_app_symbols.h"
/* named: FUN_000250c4 */
/* Reconstructed FUN_000250c4 @ 0x250c4  (parity: 300/300 trials, PROVEN) */

unsigned int FUN_000250c4(unsigned char *param_1, unsigned int *param_2)
{
  if (param_1 != 0 && param_2 != 0) {
    *param_1 = 8;
    *(unsigned int*)(param_1+4) = ((uintptr_t)&tbl_7c38b) /*=0x7c39d*/;
    *(unsigned int*)(param_1+8) = ((uintptr_t)&rodata_24ad9) /*=0x24ad9*/;
    *(unsigned int*)(param_1+0xc) = ((uintptr_t)&tbl_24fe0) /*=0x25059*/;
    *(unsigned int*)(param_1+0x10) = param_2[0];
    *(unsigned int*)(param_1+0x14) = param_2[1];
    param_1[0x18] = *((unsigned char*)param_2+8);
    return 0;
  }
  return 7;
}

