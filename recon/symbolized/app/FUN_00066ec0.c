...
void FUN_00066ec0(int *param_1, uint32_t *param_2)
{
  ...
  int iVar7 = ((uintptr_t)&g_nrfx_spim_cb) /*=0x2000b384*/;
  uint32_t uVar9 = *(volatile uint32_t*)(p2 + 0x14);
  uint32_t uVar14 = *(volatile uint8_t*)(p1 + 4);
  uint32_t uVar13;

  if (uVar9 == 0x001e8480UL) { uVar13 = ((uintptr_t)&g_sram_base_word) /*=0x20000000*/; }
  else if (uVar9 <= 0x001e8480UL) {
    if (uVar9 == ((uintptr_t)&rodata_7a120) /*=0x7a120*/) { uVar13 = 0x08000000UL; }
    else if (uVar9 <= ((uintptr_t)&rodata_7a120) /*=0x7a120*/) {
      if (uVar9 == ((uintptr_t)&rodata_1e848) /*=0x1e848*/) { uVar13 = 0x02000000UL; }
      else {
        if (uVar9 != ((uintptr_t)&rodata_3d090) /*=0x3d090*/) goto FAIL;
        uVar13 = 0x04000000UL;
      }
    } else {
      if (uVar9 != "size" /*=0xf4240*/) goto FAIL;
      uVar13 = 0x10000000UL;
    }
  }
...
FAIL:
  for (;;) {
    printk(((uintptr_t)&rodata_f7a30) /*=0xf7a30*/,"ASSERTION FAIL [%s] @ %s:%d\n" /*=0x99cbd*/,"WEST_TOPDIR/modules/hal/nordic/nrfx/drivers/src/nrfx_spim.c" /*=0xf6df0*/,0x15c);
    assert_post_action("WEST_TOPDIR/modules/hal/nordic/nrfx/drivers/src/nrfx_spim.c" /*=0xf6df0*/,0x15c);
    if (uVar9 == "size" /*=0xf4240*/) break;
  }
  uVar13 = 0x10000000UL;
SWITCH_DONE:;
