...
void FUN_0004d944(uintptr_t owner, uint32_t descriptor, uintptr_t source, uintptr_t arg4)
{
    ...
    if (!encoded) {
        ...
        if ((uint32_t)length >= 0x800u) {
            ...
            log->limit = 0x7ffu;
            log->code = 0x200u;
            FUN_0004d944(((unsigned long)&rodata_881e0) /*=0x881e0*/, 0x2480u, (uintptr_t)log, 0);
            return;
        }
        ...
        if (allocation) {
            ...
            state.zero = 0;
            int32_t rc = FUN_0004abc0(source, requested, ADDR_z_cbprintf_cpy_THUMB /*=0x7ee49*/, &state,
                                      10, scratch, 4);
            if (rc < 0) {
