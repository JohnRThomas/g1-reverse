 ...
 * public-name: img_mgmt_state_set_pending
...
int img_mgmt_state_set_pending(unsigned int request, int mode)
{
    ...
    if (status != 0) {
        struct failure_record failure = {
            6, ((unsigned long)&rodata_f2772) /*=0xf2772*/, status, request, primary, mode
        };
        FUN_0004d944(((unsigned long)&rodata_88208) /*=0x88208*/, 0x3040, &failure, 0);
        if (status == 1)
            status = 0x0c;
        else if (status == 4)
            status = 0x1d;
