...
void nrfx_twim_enable(uintptr_t *device,uint32_t a,uint32_t b,uint32_t context){
 ...
 if(state[0x22]!=1){printk(((unsigned long)"ASSERTION FAIL [%s] @ %s:%d\n") /*=0x99cbd*/,((unsigned long)&rodata_f7052) /*=0xf7052*/,((unsigned long)&rodata_f6fef) /*=0xf6fef*/,0x16b,context);assert_post_action(((unsigned long)&rodata_f6fef) /*=0xf6fef*/,0x16b);}
