...
{
    volatile uint32_t *piVar1 = (volatile uint32_t*)((uintptr_t)&g_whitelist_app_parse_buf) /*=0x20009fbc*/;
    uint32_t iVar2;
        ...
        *piVar1 = iVar2;
        if (iVar2 == 0) {
            if (*(volatile uint32_t*)((uintptr_t)&g_log_use_alt_sink) /*=0x20007554*/ == 0) {
                DEBUG_PRINT("[%s-%d] malloc failed !!\n" /*=0xa7c9c*/, "put_whitelist_app_from_app" /*=0xa8a6f*/, 0x1caUL);
            } else {
                debug_print();
            }
        } else {
    ...
    *(uint8_t*)(param_5 + 1) = (uint8_t)iVar2;
    ...
    if (iVar2 == 0) {
        ...
        if (iVar2 == 0) {
            iVar2 = *(volatile uint32_t*)((uintptr_t)&g_log_use_alt_sink) /*=0x20007554*/;
            format_string = "invalid whilte list json ! \n" /*=0xa89e8*/;
        } else {
            iVar2 = store_whitelist();
            if (iVar2 == 0) goto LAB_cleanup;
            iVar2 = *(volatile uint32_t*)((uintptr_t)&g_log_use_alt_sink) /*=0x20007554*/;
            format_string = "store_write#whitelist_app: FAIL" /*=0xa89c8*/;
        }
        if (iVar2 == 0) {
