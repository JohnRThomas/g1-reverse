...
uint update_persist_task_status(int param_1, uint param_2, uint param_3){
    volatile char* pcVar2=(volatile char*)((uintptr_t)&g_persist_task_status_lock) /*=0x20018d9c*/;
...
elseb:
    ...
    if(param_3==2){
        ...
        if((int)(((uint)((uintptr_t)&rodata_10050) /*=0x10050*/>>(param_2&0xff))<<0x1f) >= 0) goto L8a;
        goto L4a;
    }
L9e:
    if(param_3==3){
        if(*(volatile int*)((uintptr_t)&g_log_level) /*=0x2000230c*/>0){
            if(*(volatile int*)((uintptr_t)&g_log_use_alt_sink) /*=0x20007554*/==0) DEBUG_PRINT("%s(): persist task [pause]: %d\n" /*=0xa25b9*/,"update_persist_task_status" /*=0xa2660*/,param_2); else debug_print();
        }
    } else {
        if(*(volatile int*)((uintptr_t)&g_log_level) /*=0x2000230c*/>0){
            if(*(volatile int*)((uintptr_t)&g_log_use_alt_sink) /*=0x20007554*/==0) DEBUG_PRINT("%s(): persist task [stop]: %d\n" /*=0xa25d9*/,"update_persist_task_status" /*=0xa2660*/,param_2); else debug_print();
        }
    }
    goto Ltail;
L4a:
    if(*(volatile int*)((uintptr_t)&g_log_level) /*=0x2000230c*/>0){
        if(*(volatile int*)((uintptr_t)&g_log_use_alt_sink) /*=0x20007554*/==0) DEBUG_PRINT("%s(): persist task [running]: %d\n" /*=0xa2597*/,"update_persist_task_status" /*=0xa2660*/,param_2); else debug_print();
    }
    if(param_2==0x10){
        char cVar1=*(volatile char*)(param_1+0xf98);
        *(volatile char*)((uintptr_t)&g_persist_task_status) /*=0x20018d9d*/=cVar1;
