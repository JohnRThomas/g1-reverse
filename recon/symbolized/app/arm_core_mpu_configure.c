...
/* named: arm_core_mpu_configure */
/* Reconstructed arm_core_mpu_configure @ 0x50c54  (parity: 300/300 trials, PROVEN) */
#include <stdint.h>
...
extern int mpu_region_alloc_fail_log(void);
...
LOOP:
    ...
    if ((S9c & 0xffffffe0) == uVar7) {
        ...
        if (uVar5 == uVar8) {
            ...
            uVar4 = (iVar3 + 1U) & 0xff;
            if (7 < uVar4) {
                iVar3 = mpu_region_alloc_fail_log();
                if (iVar3 == -0x16) return 0xffffffea;
            } else {
                iVar3 = region_init(uVar4, (int)(intptr_t)logbuf);
            }
        }
    }
    param_3 = iVar3 + 1;
NEXT:
    ...
    goto LOOP;
DO_LOG:
    FUN_0004d944(((uintptr_t)&tbl_880d8) /*=0x88228*/, 0x1840, (int)(intptr_t)logbuf, 0);
    return 0xffffffea;
}
