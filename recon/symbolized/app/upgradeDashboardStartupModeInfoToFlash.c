...
/* named: upgradeDashboardStartupModeInfoToFlash */
/* globals referenced:
...
unsigned int upgradeDashboardStartupModeInfoToFlash(unsigned int param_1, unsigned int param_2, unsigned int param_3)
{
    ...
    unsigned int uVar2;
    (void)param_2;
    buf[0] = ((uintptr_t)&tbl_ffc8) /*=0x10003*/;
    ...
    if (iVar1 == 0) {
        if (2 < *(volatile int*)((uintptr_t)&g_log_level) /*=0x2000230c*/) {
            if (*(volatile int*)((uintptr_t)&g_log_use_alt_sink) /*=0x20007554*/ == 0)
                DEBUG_PRINT("%s(): send startup mode  store Command, mode = %d\n" /*=0x9e98c*/, "upgradeDashboardStartupModeInfoToFlash" /*=0x9ed68*/, param_1);
            else
                debug_print();
        }
        ...
        if (0 < *(volatile int*)((uintptr_t)&g_log_level) /*=0x2000230c*/) {
            if (*(volatile int*)((uintptr_t)&g_log_use_alt_sink) /*=0x20007554*/ == 0)
                DEBUG_PRINT("%s(): setting flash store queue fill failed\r\n\n" /*=0x9e903*/, "upgradeDashboardStartupModeInfoToFlash" /*=0x9ed68*/);
            else
                debug_print();
        }
        uVar2 = 0xffffffffUL;
