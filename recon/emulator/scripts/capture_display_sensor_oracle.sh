...
# P4 iteration 41: Renode's EMULATION-WIDE PRNG seed was the missing
# determinism knob.  platforms/nrf5340.repl instantiates a stock
# `Miscellaneous.NRF52840_RNG` for the net core at 0x41009000, which draws from
# `Emulation.RandomGenerator` -- re-seeded RANDOMLY at every Renode start
# (measured: 2124439726 then 720424243 on two bare launches).  The CPUNET BLE
# controller pulls entropy through it, so every capture got a different radio
# cadence: a sub-microsecond divergence at t ~ 2 s that amplified into a 100 ms
# scheduling shift by t ~ 3.9 s.  Consequence, measured on the SHIPPED images:
# two runs gave spim_a 786/2859 and 764/2881 (whole run 3645 both).
# With this seed pinned, two shipped runs are BYTE-IDENTICAL in every trace
# file, and they reproduce the RECORDED oracle exactly
...
# The emulation-wide PRNG seed MUST be pinned BEFORE the platform is created,
# because the net core's stock NRF52840_RNG draws from it (see G1_SEED above).
emulation SetSeed $G1_SEED
...
# LocalTimeSource.ExecuteInSerial is True as well -- verified live, iteration 41)
# ===========================================================================
echo "ORACLE_EMULATION_SEED: $G1_SEED"
