...
int sdc_llcp_process_rx_control_pdu(const uint8_t *pdu)
{
    ...
    *active_link_index = sdc_llcp_get_active_link_index();
    ...
    if (opcode == 7) { /* LL_UNKNOWN_RSP */
        ...
        case 3:
        case 7:
            G1_NORETURN_CALL(sdc_llcp_return_tail((uint32_t)result,
                                 (uint32_t)result << 4,
                                 LLCP_UNKNOWN_RESPONSE_TABLE, state_index));
        case 1:
            G1_NORETURN_CALL(sdc_llcp_store_unknown_result_tail(
        ...
        case 5:
        case 6:
            G1_NORETURN_CALL(sdc_llcp_continue_procedure_tail(
                (uint32_t)result, (uint32_t)result << 4,
                LLCP_UNKNOWN_RESPONSE_TABLE, state_index));
        default:
            G1_NORETURN_CALL(sdc_assertion_fail(0x32, 0x13b3));
        }
