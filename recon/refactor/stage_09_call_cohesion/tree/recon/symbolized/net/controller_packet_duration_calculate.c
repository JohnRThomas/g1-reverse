...
extern uint32_t controller_packet_duration_lookup(uint32_t product, uint32_t flags, uint32_t length,
                      ...
                      uint32_t unused_r2, uint32_t preserved_r3)
{
    ...
    if ((flags & 0x10u) != 0u) {
        return controller_packet_duration_lookup(product, flags, 0x28u, preserved_r3);
    }
