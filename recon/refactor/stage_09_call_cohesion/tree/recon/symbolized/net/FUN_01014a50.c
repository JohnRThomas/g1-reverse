#include "g1_net_symbols.h"
/* readable reconstruction; identity: FUN_01014a50 @ 0x01014a50
 * public-name: FUN_01014a50
 * durable-map: recon/catalogs/function_names_net.json
 * callees (readable <= raw @ address):
 *   controller_timing_window_is_large        <= FUN_0100e7e0 @ 0x0100e7e0
 *   controller_timing_windows_store          <= FUN_0100f198 @ 0x0100f198
 *   controller_packet_result_prepare         <= FUN_0100f2d0 @ 0x0100f2d0
 *   controller_procedure_generation_advance  <= FUN_0100f798 @ 0x0100f798
 *   controller_procedure_slot_reserve        <= FUN_0100f834 @ 0x0100f834
 * address symbols (name @ address):
 *   g_net_link_ctx_a                         @ 0x21000f68
 */
/* net-core FUN_01014a50 @ 0x1014a50  (parity 300 trials PROVEN) */
#include <stdint.h>

extern int FUN_0100e7d0(unsigned int a);
extern int FUN_0100e7d4(unsigned int a);
extern int FUN_0100e7d8(unsigned int a);
extern int FUN_0100e7dc(unsigned int a);
extern int controller_timing_window_is_large(unsigned int a);
extern void controller_timing_windows_store(int a, int b, int c, int d, int e);
extern void controller_packet_result_prepare(unsigned short a, void *b, void *c);
extern void controller_procedure_generation_advance(int a);
extern unsigned int controller_procedure_slot_reserve(int a, unsigned int b);

#define DAT_b14 ((unsigned long)&g_net_link_ctx_a) /*=0x21000f68*/

unsigned int FUN_01014a50(int param_1, unsigned int param_2)
{
    int iVar1;
    int uVar2, uVar3, uVar4, uVar5;

    iVar1 = controller_timing_window_is_large(param_2);
    if (iVar1 == 0) {
        return controller_procedure_slot_reserve(param_1, param_2);
    }

    iVar1 = *(volatile int *)(param_1 + 4);
    uVar2 = FUN_0100e7d8(param_2);
    uVar3 = FUN_0100e7d0(param_2);
    uVar4 = FUN_0100e7dc(param_2);
    uVar5 = FUN_0100e7d4(param_2);
    controller_timing_windows_store(iVar1 + 0x30, uVar2, uVar3, uVar4, uVar5);

    if ((int)((unsigned int)(*(volatile unsigned short *)(DAT_b14 + 0xc)) << 0x1e) < 0 &&
        *(volatile unsigned char *)(*(volatile int *)(DAT_b14 + 0x10) + 3) == 0x14) {
        iVar1 = *(volatile int *)(param_1 + 4);
        *(volatile unsigned char *)(iVar1 + 100) = 1;
    } else {
        iVar1 = *(volatile int *)(param_1 + 4);
        if (*(volatile unsigned char *)(iVar1 + 0xc5) == 0x1e) {
            *(volatile unsigned char *)(iVar1 + 100) = 1;
        } else if (*(volatile unsigned char *)(iVar1 + 100) != 1 &&
                   *(volatile unsigned char *)(iVar1 + 0x163) == 0 &&
                   *(volatile unsigned char *)(iVar1 + 0x162) == 0) {
            unsigned short *puVar6 = *(volatile unsigned short **)(DAT_b14 + 4);
            *(volatile unsigned int *)(iVar1 + 0xb4) = *(volatile unsigned int *)(iVar1 + 0xb4) | 0x20;
            controller_packet_result_prepare(*puVar6, puVar6 + 0x18, puVar6 + 0xf1);
            controller_procedure_generation_advance(param_1);
            return 0;
        } else {
            *(volatile unsigned int *)(iVar1 + 0xb4) = *(volatile unsigned int *)(iVar1 + 0xb4) | 0x10;
            controller_procedure_generation_advance(param_1);
            return 0;
        }
    }
    *(volatile unsigned int *)(iVar1 + 0xb4) = *(volatile unsigned int *)(iVar1 + 0xb4) | 0x10;
    controller_procedure_generation_advance(param_1);
    return 0;
}
