...
unsigned smp_error(int param_1, unsigned param_2){
    ...
    if (uVar1 == 0){
        ...
        *(volatile unsigned char *)puVar3 = (unsigned char)param_2;
        int i = bt_conn_create_pdu(*(volatile unsigned int *)(param_1 + 0xf0), 6, node, 0, 0);
        if (i != 0) net_buf_unref(node);
    } else {
        loc[1] = ((unsigned long)&rodata_f4f98) /*=0xf4f98*/; loc[0] = 2;
        smp_log_message(((unsigned long)&rodata_88180) /*=0x88180*/, 0x1080, loc);
        bt_conn_disconnect_by_state(*(volatile unsigned int *)(param_1 + 0xf0), 5);
    }
    return 0;
}
