 ...
 *   rodata_a0354                             @ 0x000a0354   [INLINED -- refactor stage 01]
 *   rodata_a036a                             @ 0x000a036a   [INLINED -- refactor stage 01]
 *   rodata_a0396                             @ 0x000a0396   [INLINED -- refactor stage 01]
 *   rodata_a03b3                             @ 0x000a03b3
 *   rodata_a03d9                             @ 0x000a03d9   [INLINED -- refactor stage 01]
 *   rodata_a03f5                             @ 0x000a03f5   [INLINED -- refactor stage 01]
 *   rodata_a0434                             @ 0x000a0434   [INLINED -- refactor stage 01]
 *   rodata_a044e                             @ 0x000a044e   [INLINED -- refactor stage 01]
 *   rodata_a0477                             @ 0x000a0477   [INLINED -- refactor stage 01]
 *   rodata_a04cb                             @ 0x000a04cb   [INLINED -- refactor stage 01]
 *   rodata_a050a                             @ 0x000a050a   [INLINED -- refactor stage 01]
 *   rodata_a052d                             @ 0x000a052d   [INLINED -- refactor stage 01]
 *   rodata_a0568                             @ 0x000a0568
 *   rodata_a05c2                             @ 0x000a05c2
 *   rodata_a0621                             @ 0x000a0621
 *   rodata_a067b                             @ 0x000a067b
 *   rodata_a06d7                             @ 0x000a06d7   [INLINED -- refactor stage 01]
 *   rodata_a07eb                             @ 0x000a07eb   [INLINED -- refactor stage 01]
 *   rodata_a0889                             @ 0x000a0889   [INLINED -- refactor stage 01]
 *   rodata_a0932                             @ 0x000a0932   [INLINED -- refactor stage 01]
 *   rodata_a0962                             @ 0x000a0962   [INLINED -- refactor stage 01]
 *   rodata_a09ac                             @ 0x000a09ac   [INLINED -- refactor stage 01]
 *   rodata_a09ee                             @ 0x000a09ee
 *   rodata_a0a04                             @ 0x000a0a04   [INLINED -- refactor stage 01]
 *   rodata_a0a2e                             @ 0x000a0a2e
 *   rodata_a0a47                             @ 0x000a0a47
 *   rodata_a0a61                             @ 0x000a0a61
 *   rodata_a0a7b                             @ 0x000a0a7b
 *   rodata_a1a43                             @ 0x000a1a43   [INLINED -- refactor stage 01]
...
extern int device_addr_is_default(unsigned int, unsigned int); /* @ 0x7c132 */
...
extern void set_message_pending_state(int); /* @ 0x3443c */
...
#define DISPLAY_LOG_TAG ((uintptr_t)((unsigned long)"slave_display_thread") /*=0xa1a43*/)
                          ...
                          uint32_t param_3)
{
  ...
  context->work_mode = 0;
  LOG_IF(1, ((unsigned long)"%s(): enter\n") /*=0x99969*/, param_3, 0, context, param_2, param_3);
...
while_cond:
  ...
  case 0:
    ...
    if ((int8_t)context->work_mode != 6 && display_panel_is_secondary(context) == 0) {
      LOG_IF(0, ((unsigned long)"%s(): Ignore data...\n") /*=0xa0354*/);
      goto top;
    }
    LOG_IF(2, ((unsigned long)"%s(): resume because SYNC_DISPLAY_CMD_DATA\n") /*=0xa036a*/);
    change_work_mode_to(2);
    if ((int8_t)context->work_mode == 6 && (int8_t)context->screen_state != 2) context->screen_state = 2;
    goto Ldefault;

  case 1:
    LOG_IF(2, ((unsigned long)"%s(): ****SUSPEND start****\n") /*=0xa0396*/);
  ...
  case 2:
    LOG_IF(2, ((unsigned long)"%s(): ****RESUME start****\n") /*=0xa03d9*/);
    if ((int8_t)context->remote_command == 0xb) {
      LOG_IF(1, ((unsigned long)"%s(): slave rcv canvas_distance_gear:%d,raster_height_gear:%d\n") /*=0xa03f5*/, context->remote_value, context->remote_aux);
    ...
    *g_display_update_active = 1;
    ...
    LOG_IF(2, ((unsigned long)"%s(): ****RESUME end****\n") /*=0xa0434*/);
  ...
  case 6:
    LOG_IF(2, ((unsigned long)"%s(): received master send sync command\n") /*=0xa044e*/);
    if ((int8_t)context->remote_command == 9) {
      LOG_IF(2, ((unsigned long)"%s():  SLAVE received dashboard startup mode changed infomation ,startup mode = %d\n") /*=0xa0477*/, context->remote_value);
    ...
    case 1:
      if ((int8_t)context->work_mode == 0xc) {
        if ((int8_t)context->remote_value == 0) {
          LOG_IF(2, ((unsigned long)"%s(): QUICK NOTE SLAVE Delay EXIT\n") /*=0xa050a*/);
          display_DelayClose(10000);
          goto L2812a;
        }
      } else if ((int8_t)context->remote_value == 1) {
        LOG_IF(2, ((unsigned long)"%s(): Quick Note Slave Start up...,current screen ID = %d\n") /*=0xa052d*/, context->work_mode);
    ...
    case 2: {
      ...
      L282f6:
        LOG_IF(1, ((unsigned long)"%s(): received key press event ,slave screen id = %d,drop it ...\n") /*=0xa06d7*/, context->work_mode);
      }
      goto top;
    }
    ...
    case 4: {
      ...
      uint64_t uptime;
      LOG_IF(2, ((unsigned long)"%s(): slave sync onboarding action_cmd %d, sub_step %d, data: %d\n") /*=0xa09ac*/, context->remote_value, context->remote_aux,
             context->remote_flag);
    ...
    case 5:
      LOG_IF(2, ((unsigned long)"%s(): slave sync not_disturb sub_step %d\n") /*=0xa0a04*/, context->remote_value);
      {
    ...
    case 6:
      if ((int8_t)context->work_mode == 0xa) {
        ...
        LOG_IF(2, ((unsigned long)"%s(): navigation SLAVE disconnect EXIT\n") /*=0xa07eb*/);
      L28342:
    ...
    case 7:
      if ((int8_t)context->work_mode == 6) {
        LOG_IF(2, ((unsigned long)"%s(): dashboard SLAVE received power infomation \n") /*=0xa0889*/);
    ...
    case 8:
      LOG_IF(2, ((unsigned long)"%s():  SLAVE received app language infomation \n") /*=0xa0932*/);
      cv = (int8_t)context->remote_value;
      if (*(volatile int8_t *)g_remote_language != cv) LOG_IF(1, ((unsigned long)"%s(): The information sent by the host is different from that of the APP\n") /*=0xa0962*/);
      upgradeAppLanguageInfoToFlash((uint8_t)cv);
      goto top;

    case 10:
      LOG_IF(2, ((unsigned long)"%s():  SLAVE received dashboard double click startup command \n") /*=0xa04cb*/);
