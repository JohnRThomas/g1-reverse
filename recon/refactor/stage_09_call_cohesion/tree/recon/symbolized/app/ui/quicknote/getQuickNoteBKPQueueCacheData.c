...
static volatile int *const log_level = (volatile int *)((unsigned long)&g_log_level) /*=0x2000230c*/;
static volatile int *const deferred_logger = (volatile int *)((unsigned long)&g_log_use_alt_sink) /*=0x20007554*/;
static uint8_t *const slot_image = (uint8_t *)((unsigned long)g_quicknote_flash_page_buf) /*=0x20018465*/;
...
int getQuickNoteBKPQueueCacheData(uint32_t operation, uint32_t slot, uint32_t value)
{
    ...
    result = k_msgq_get((void *)((unsigned long)&g_quicknote_flash_msgq) /*=0x20003960*/, &request, 0, 0);
    if (result != 0) {
        if (*log_level < 1)
            return -1;
        if (*deferred_logger == 0)
    ...
    if (slot_number > 3) {
        if (*log_level < 1)
            return -1;
        log_fixed(((unsigned long)&rodata_9ee54) /*=0x9ee54*/);
        return -1;
    }
    ...
    if (z_device_is_ready((const void *)((unsigned long)&rodata_87bf0) /*=0x87bf0*/) == 0) {
        if (*log_level > 0)
            log_value(((unsigned long)&rodata_9e9ea) /*=0x9e9ea*/, *(volatile uint32_t *)((unsigned long)&rodata_87bf0) /*=0x87bf0*/);
        return -1;
    }
    ...
    if (result != 0) {
        if (*log_level > 0)
            log_value(((unsigned long)&rodata_9ea0a) /*=0x9ea0a*/, slot_flash_address);
        return -1;
    }
    ...
    if (result != 0) {
        if (*log_level > 0)
            log_value(result == -1 ? ((unsigned long)&rodata_9ebc6) /*=0x9ebc6*/ : ((unsigned long)&rodata_9eba7) /*=0x9eba7*/,
                      (uint32_t)result);
        return result;
    }
