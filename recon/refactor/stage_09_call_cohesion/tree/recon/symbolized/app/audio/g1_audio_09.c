 ...
 *   sendAudioStram2Cache.c
 *   get_audio_msgq_used_count.c
 */
...
uint32_t sendAudioStram2Cache(const void *audio_stream)
{
    ...
    if (*(volatile int *)((uint8_t *)AUDIO_CACHE_QUEUE + 0x24) == 0x12) {
        if (LOG_LEVEL > 0) {
            G1_LOG_ROUTE(DEFERRED_LOGGER_ENABLED == 0, LOG_CACHE_FULL, LOG_MODULE_AUDIO_CACHE);
        }
        return 0xffffffffu;
    }
    ...
    if (LOG_LEVEL > 0) {
        G1_LOG_ROUTE(DEFERRED_LOGGER_ENABLED == 0, LOG_CACHE_SEND_FAILED, LOG_MODULE_AUDIO_CACHE);
    }
    return 0xffffffffu;
}
