...
void put_whitelist_app_from_app(uint32_t param_1, uint32_t param_2, uint32_t param_3, uint32_t param_4, uint32_t param_5)
{
    ...
    if (*piVar1 == 0) {
        ...
        *piVar1 = iVar2;
        if (iVar2 == 0) {
            G1_LOG_ROUTE(*(volatile uint32_t *)((unsigned long)&g_log_use_alt_sink) /*=0x20007554*/ == 0, ((unsigned long)"[%s-%d] malloc failed !!\n") /*=0xa7c9c*/, ((unsigned long)"put_whitelist_app_from_app") /*=0xa8a6f*/, 0x1caUL);
        } else {
    ...
    *(uint8_t*)(param_5 + 1) = (uint8_t)iVar2;
    ...
    if (iVar2 == 0) {
        ...
        G1_LOG_ROUTE(iVar2 == 0, format_string, 0, 0);
