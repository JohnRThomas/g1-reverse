                      ...
                      uint8_t *unread_count)
{
    ...
    if (notification == 0 || unread_count == 0) {
        ...
        if (level > 1) {
            ...
            G1_LOG_ROUTE(sink == 0U, ((unsigned long)&rodata_a7cd6) /*=0xa7cd6*/, ((unsigned long)&rodata_a829f) /*=0xa829f*/);
        }
        return 0;
    }
    ...
    *unread_count = count;
    ...
    if (level > 2) {
        sink = *(volatile uint32_t *)((unsigned long)&g_log_use_alt_sink) /*=0x20007554*/;
        G1_LOG_ROUTE(sink == 0U, ((unsigned long)&rodata_a7cf4) /*=0xa7cf4*/, ((unsigned long)&rodata_a829f) /*=0xa829f*/, count);
    }
    return 1;
}
