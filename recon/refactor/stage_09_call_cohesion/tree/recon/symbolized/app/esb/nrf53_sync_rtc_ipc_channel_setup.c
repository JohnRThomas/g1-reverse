...
void nrf53_sync_rtc_ipc_channel_setup(void *object, uint32_t context, uint32_t descriptor)
{
    ...
    if (prepare == 0 || prepare(object, context, 0) != 0) {
        G1_ASSERT_FAIL((((unsigned long)"ASSERTION FAIL [%s] @ %s:%d\n") /*=0x99cbd*/, ((unsigned long)"err == 0") /*=0xa7a10*/, ((unsigned long)"WEST_TOPDIR/zephyr/soc/arm/nordic_nrf/nrf53/sync_rtc.c") /*=0xf0a90*/, 0xcbu), (((unsigned long)"WEST_TOPDIR/zephyr/soc/arm/nordic_nrf/nrf53/sync_rtc.c") /*=0xf0a90*/, 0xcbu));
    }
    ...
    if (slot >= 16) {
        G1_ASSERT_FAIL((((unsigned long)"ASSERTION FAIL [%s] @ %s:%d\n") /*=0x99cbd*/, ((unsigned long)"index < 16") /*=0xf0a5d*/, ((unsigned long)"WEST_TOPDIR/modules/hal/nordic/nrfx/hal/nrf_ipc.h") /*=0xf0a2b*/, 0x1e1u), (((unsigned long)"WEST_TOPDIR/modules/hal/nordic/nrfx/hal/nrf_ipc.h") /*=0xf0a2b*/, 0x1e1u));
    }
