...
void power_manager_restrict_event_log(const indexed_event *event)
{
    ...
    if (event->index >= 2) {
        G1_ASSERT_FAIL((((unsigned long)"ASSERTION FAIL [%s] @ %s:%d\n") /*=0x99cbd*/, ((unsigned long)"event->module_idx < module_count()") /*=0xf18bd*/, ((unsigned long)"WEST_TOPDIR/nrf/subsys/caf/events/power_manager_event.c") /*=0xf1885*/, 0x15u), (((unsigned long)"WEST_TOPDIR/nrf/subsys/caf/events/power_manager_event.c") /*=0xf1885*/, 0x15u));
    }
    if (event->operation > 2) {
        G1_ASSERT_FAIL((((unsigned long)"ASSERTION FAIL [%s] @ %s:%d\n") /*=0x99cbd*/, ((unsigned long)"lvl <= POWER_MANAGER_LEVEL_MAX") /*=0xf18e0*/, ((unsigned long)"WEST_TOPDIR/nrf/subsys/caf/events/power_manager_event.c") /*=0xf1885*/, 0x16u), (((unsigned long)"WEST_TOPDIR/nrf/subsys/caf/events/power_manager_event.c") /*=0xf1885*/, 0x16u));
    }
    if (event->operation < -1) {
        G1_ASSERT_FAIL((((unsigned long)"ASSERTION FAIL [%s] @ %s:%d\n") /*=0x99cbd*/, ((unsigned long)"lvl >= POWER_MANAGER_LEVEL_ALIVE") /*=0xf18ff*/, ((unsigned long)"WEST_TOPDIR/nrf/subsys/caf/events/power_manager_event.c") /*=0xf1885*/, 0x17u), (((unsigned long)"WEST_TOPDIR/nrf/subsys/caf/events/power_manager_event.c") /*=0xf1885*/, 0x17u));
    }
