...
uint32_t gui_area_clear(uint32_t param_1, uint32_t param_2, uint32_t param_3, uint32_t param_4)
{
    ...
    if (*piVar1 > 2) {
        G1_LOG_ROUTE(*(volatile uint32_t *)((unsigned long)&g_log_use_alt_sink) /*=0x20007554*/ == 0, ((unsigned long)"%s(): exec area clear command....\n") /*=0xaa8c8*/, ((unsigned long)"gui_area_clear") /*=0xaae00*/);
    }
    iVar2 = get_projector_controller();
    if (*(uint32_t*)(iVar2 + 0x35c) == 0) {
        if (*piVar1 > 0) {
            G1_LOG_ROUTE(*(volatile uint32_t *)((unsigned long)&g_log_use_alt_sink) /*=0x20007554*/ == 0, ((unsigned long)"%s(): c->init_done = 0,init failed, can't write data!\n") /*=0xaa891*/, ((unsigned long)"gui_area_clear") /*=0xaae00*/);
        }
        uVar3 = 0xffffffff;
    } else {
