...
static volatile int32_t *const log_level =
    (volatile int32_t *)((unsigned long)&g_log_level) /*=0x2000230c*/; /* g_log_level */
static volatile int32_t *const use_alternate_log_sink =
    (volatile int32_t *)((unsigned long)&g_log_use_alt_sink) /*=0x20007554*/; /* g_log_use_alt_sink */
static volatile uint8_t *const glyph_dither_mask =
    (volatile uint8_t *)((unsigned long)&g_gui_dark_light_dither_mask) /*=0x200034f6*/; /* g_gui_dark_light_dither_mask */
                               ...
                               int32_t skip_builtin_rendering)
{
    ...
    if (callback != 0) {
        if (*log_level > 1) {
            G1_LOG_ROUTE(*use_alternate_log_sink == 0, ((unsigned long)"%s(): exex process effect callback function\n") /*=0xaaa7f*/, ((unsigned long)"gui_utf_draw_truncate") /*=0xaad78*/, 0, 0);
        }
    ...
    for (;;) {
        ...
        if (character_class == 0) {
            ...
            if (status < 0) {
                if (*log_level > 1) {
                    G1_LOG_ROUTE(*use_alternate_log_sink == 0, ((unsigned long)&rodata_aaa58) /*=0xaaa58*/, ((unsigned long)"gui_utf_draw_truncate") /*=0xaad78*/,
                                    codepoint, 0);
                }
            } else {
                ...
                if ((uint32_t)(right - (int32_t)left) < measured_width) {
                    ...
                    if (maximum_lines <= line_count ||
                        ...
                        while (rewind_index != 0) {
                            ...
                            if (status < 0) {
                                if (*log_level > 1) {
                                    G1_LOG_ROUTE(*use_alternate_log_sink == 0, ((unsigned long)"%s(): truncate can not find [%d]0x%x font resource\n") /*=0xaab9e*/,
                        ...
                        if (*log_level > 1) {
                            G1_LOG_ROUTE(*use_alternate_log_sink == 0, ((unsigned long)"%s(): end line x0=%d\n") /*=0xaabd2*/, ((unsigned long)"gui_utf_draw_truncate") /*=0xaad78*/,
                                            x_offset, 0);
                        }
                        status = resource_manger_get(
