...
int imu_fusion_init(unsigned char *context, uint32_t p2, uint32_t p3)
{
    ...
    if (context[0x14] != 0) {
        if (LOG_LEVEL > 1) {
            sink = LOG_SINK;
            G1_LOG_ROUTE(sink == 0, ((unsigned long)"%s(): imu_fusion: algo is existed\n\n") /*=0x9f773*/, LOG_TAG, p3, (uint32_t)sink);
        }
        return -1;
    }
    ...
    *(volatile uint32_t *)(context + 0x1c) = ((unsigned long)&rodata_87d58) /*=0x87d58*/;
    if (LOG_LEVEL > 1) {
        sink = LOG_SINK;
        G1_LOG_ROUTE(sink == 0, ((unsigned long)"%s(): imu_fusion: sensor device is ready:\n\n") /*=0x9f797*/, LOG_TAG, p3, (uint32_t)sink);
    }
    ...
    *(volatile uint32_t *)(context + 0x24) = 0u;
    ...
    if (rc < 0) {
        if (LOG_LEVEL > 1) {
            sink = LOG_SINK;
            G1_LOG_ROUTE(sink == 0, ((unsigned long)"%s(): c->imu_fusion: Cannot set sampling frequency for accelerometer ret %d.\n\n") /*=0x9f7c3*/, LOG_TAG, (uint32_t)rc, (uint32_t)sink);
        }
        return -3;
    }
    ...
    *(volatile uint32_t *)(context + 0x24) = 0u;
    ...
    if (rc < 0) {
        if (LOG_LEVEL > 1) {
            sink = LOG_SINK;
            G1_LOG_ROUTE(sink == 0, ((unsigned long)"%s(): c->imu_fusion: Cannot set sampling frequency for gyro.\n\n") /*=0x9f812*/, LOG_TAG,
    ...
    *(volatile uint32_t *)(context + 0x18) = 0u;
    if (LOG_LEVEL > 1) {
        sink = LOG_SINK;
        G1_LOG_ROUTE(sink == 0, ((unsigned long)"%s(): sensor_fusion_init:\n\n") /*=0x9f851*/, LOG_TAG,
                        (uint32_t)(unsigned long)(context + 0x20), (uint32_t)sink);
    }
