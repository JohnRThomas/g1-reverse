...
int SlaveDoubleClickEventInject(void)
{
  ...
  if (*(volatile int32_t *)((unsigned long)&g_log_level) /*=0x2000230c*/ > 2) {
    G1_LOG_ROUTE(*(volatile uint32_t *)((unsigned long)&g_log_use_alt_sink) /*=0x20007554*/ == 0, ((unsigned long)"%s(): received slave send double click event ,boot or close dashboard function\n") /*=0xef644*/, ((unsigned long)"SlaveDoubleClickEventInject") /*=0xef694*/);
  }
  return 0;
}
