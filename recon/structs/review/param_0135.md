## review group param_0135  (1 cluster(s), kinds=param)

### proposal for param_0135
struct_name: nvs_fs | is_library: True | library_name: nvs_fs | is_array: None | confidence: high
purpose: Zephyr NVS filesystem/sector context object shared across nvs_mount/nvs_write/nvs_read_hist/sector-advance/gc-done routines
fields:
  0x0  uint32_t     offset  
  0x4  uint16_t     ate_wra  overlaps subsequent field per layout notes; write allocation table entry address (lo)
  0x8  uint32_t     data_wra  
  0xc  uint16_t     sector_size  
  0x10  uint8_t      sector_count  
  0x28  uint32_t     flash_parameters_or_lock  
  0x2c  void *       flash_device  

<ground-truth bundle for param_0135>
