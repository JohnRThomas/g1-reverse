## review group param_0702  (1 cluster(s), kinds=param)

### proposal for param_0702
struct_name: virtqueue | is_library: True | library_name: virtqueue (libmetal/open-amp) | is_array: None | confidence: medium
purpose: OpenAMP/libmetal virtqueue instance read by virtqueue_get_buffer: queue index (u16), ring length (int), free/queued counters (int), and small ring bookkeeping fields (avail/used indices).
fields:
  0xa  uint16_t     vq_queue_index  
  0x18  int32_t      vq_nentries  
  0x20  int32_t      vq_free_cnt  
  0x24  uint16_t     vq_ring_avail_idx  
  0x2c  uint16_t     vq_ring_used_idx  
  0x2e  uint16_t     vq_queued_cnt  

<ground-truth bundle for param_0702>
