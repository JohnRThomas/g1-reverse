## review group param_0438  (1 cluster(s), kinds=param)

### proposal for param_0438
struct_name: mbedtls_ctr_drbg_context | is_library: True | library_name: mbedtls_ctr_drbg_context | is_array: None | confidence: high
purpose: mbedTLS CTR-DRBG (deterministic random bit generator) context: AES cipher context, counter, reseed state, entropy callback and personalization data
fields:
  0x4  uint32_t     counter_word0  
  0x8  uint32_t     counter_word1  
  0xc  uint32_t     counter_word2  
  0x10  int          reseed_counter  
  0x14  int          prediction_resistance  
  0x18  size_t       entropy_len  
  0x1c  int          reseed_interval  
  0x20  uint32_t     aes_ctx_word0  
  0x24  uint32_t     aes_ctx_word1  
  0x28  uint32_t     aes_ctx_word2  
  0x2c  uint32_t     aes_ctx_word3  
  0x30  uint32_t     aes_ctx_word4  
  0x34  uint32_t     aes_ctx_word5  
  0x38  uint32_t     aes_ctx_word6  
  0x3c  uint32_t     aes_ctx_word7  
  0x94  int (*)(void *, unsigned char *, size_t) f_entropy  entropy callback function pointer
  0x98  void *       p_entropy  entropy context pointer (or continuation of entropy_len field)

<ground-truth bundle for param_0438>
