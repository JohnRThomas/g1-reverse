## review group param_0713  (1 cluster(s), kinds=param)

### proposal for param_0713
struct_name: rpmsg_virtio_shm_pool | is_library: True | library_name: rpmsg_virtio_shm_pool | is_array: None | confidence: medium
purpose: OpenAMP rpmsg_virtio shared-memory pool descriptor (base address and size) used to init the shm pool
fields:
  0x4  void *       base  
  0x8  size_t       size  

<ground-truth bundle for param_0713>
### cluster param_0713  (param, 1 members, 2 fields, size>=0xc)

Shared pointer-parameter object unified across call-argument flow.

library hint: LIKELY LIBRARY (prefixes: -; 1/1 members are LIBRARY-class)

members (1 shown of 1):
  - rpmsg_virtio_init_shm_pool @ 0x85be8  as param_1  [LIBRARY]

candidate layout (offsets/sizes are GROUND TRUTH from Ghidra — do not change them; name the struct + fields, refine types, judge cohesion):
```c
struct param_0713 {
    uint8_t    _pad_0x0[0x4];   /* +0x0 pad */
    uint32_t   field_0x4;   /* +0x4  sz=4 rw=r types=idx4 */
    uint32_t   field_0x8;   /* +0x8  sz=4 rw=r types=idx4 */
};
```
