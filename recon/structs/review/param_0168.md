## review group param_0168  (1 cluster(s), kinds=param)

### proposal for param_0168
struct_name: k_thread | is_library: True | library_name: k_thread | is_array: None | confidence: high
purpose: Zephyr thread control block (k_thread/z_thread_base) accessed by scheduler internals: MPU region config, priority, state flags, timeout node, and wait-queue linkage
fields:
  0x4  uint32_t     field_0x4  idx4, possibly MPU region idx
  0x8  uint32_t     field_0x8  rw int
  0xc  uint8_t      prio  read-only, thread priority
  0xd  uint8_t      sched_locked_or_state  rw byte
  0xe  uint8_t      user_options_or_state  rw byte
  0x18  uint32_t     field_0x18  idx4, likely timeout dlist node
  0x58  uint32_t     field_0x58  read-only int, likely swap_retval or wait_q linkage

<ground-truth bundle for param_0168>
