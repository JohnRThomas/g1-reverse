## review group param_0038  (1 cluster(s), kinds=param)

### proposal for param_0038
struct_name: app_shared_system_ctx | is_library: False | library_name:  | is_array: None | confidence: low
purpose: Large shared application context/state block passed across BLE ANCS, audio, DMIC and notification subsystems (message queue node + status/flag byte fields)
fields:
  0xc  void*        field_0xc  pointer, read-only
  0x10  void*        field_0x10  pointer, read-write, possibly next-node link
  0x14  uint32_t     field_0x14  rw uint, possibly length/count
  0x3c  uint32_t     field_0x3c  written, status word
  0x41  uint8_t      field_0x41  written flag byte
  0x42  uint8_t      field_0x42  written flag byte
  0x44  uint8_t      field_0x44  written flag byte
  0x64  uint8_t      field_0x64  written flag byte
  0x1e4  uint32_t     field_0x1e4  written status word
  0x759  uint8_t      field_0x759  read-only byte
  0x844  uint8_t      field_0x844  read-only byte
  0x846  uint8_t      field_0x846  read-only char

<ground-truth bundle for param_0038>
