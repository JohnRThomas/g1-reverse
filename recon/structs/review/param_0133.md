## review group param_0133  (1 cluster(s), kinds=param)

### proposal for param_0133
struct_name: mpsc_pbuf_buffer | is_library: True | library_name: mpsc_pbuf_buffer | is_array: None | confidence: high
purpose: Zephyr multi-producer single-consumer packet buffer (used by logging subsystem) - vtable/notify fn ptr, write/read indices, capacity, backing buffer pointer, and utilization stats, matched by mpsc_pbuf_commit/idx_inc/rd_idx_inc/drop_item helper functions
fields:
  0x0  void (*)(void) notify_drop  function pointer (code)
  0x4  uint32_t     wr_idx  rw index
  0x8  uint32_t     rd_idx  read-only index
  0xc  uint32_t     tmp_wr_idx  rw
  0x10  uint32_t     flags  rw
  0x1c  void *       buf  backing buffer pointer, read-only
  0x20  uint32_t     size  buffer size, read-only
  0x24  uint32_t     max_usage  read-only uint
  0x28  uint32_t     utilization  rw uint

<ground-truth bundle for param_0133>
