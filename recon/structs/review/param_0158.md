## review group param_0158  (1 cluster(s), kinds=param)

### proposal for param_0158
struct_name: bt_ancs_client_ctx | is_library: False | library_name:  | is_array: None | confidence: low
purpose: Apple Notification Center Service (ANCS) client context: handle/attribute bookkeeping near the front plus a large (~0x8bb byte) attribute-data buffer and discovery/state trailer fields.
fields:
  0x2  uint16_t     field_0x2  read, small id/handle
  0x4  uint32_t     state_flags  rw state word
  0x8  uint16_t     ccc_handle  write-only handle
  0x14  uint32_t     attr_id  read as idx4
  0x1a  uint16_t     attr_len  write-only length
  0x1c  uint32_t     attr_flags  read as idx4
  0x24  uint32_t     notif_uid  read as idx4
  0x28  uint8_t      category_id  write-only
  0x29  uint8_t      category_count  write-only
  0x2a  uint8_t      event_id  write-only
  0x2b  uint8_t      event_flags  write-only
  0x2c  uint8_t      client_state  write-only
  0x8e8  uint8_t      attr_buf_flag  read-only, follows large attribute buffer region
  0x8f8  uint32_t     attr_buf_len  rw
  0x904  uint32_t     discover_state  write-only
  0x908  uint8_t      conn_active  write-only

<ground-truth bundle for param_0158>
