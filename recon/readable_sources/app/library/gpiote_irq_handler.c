 ...
 *   local_30         => struct g1_layout_gpiote_evt_info__stack_1312            [stack_1312; G1-original]
 * Raw function identity: 0x00065b9c.  See ../include/g1_recovered_layouts.h. */
...
void gpiote_irq_handler(void)
{
  uint32_t interrupt_mask =
      gpiote_read_and_clear_pending_events((*(volatile uint32_t *)REG_5000d304 /*=0x5000d304*/ & 0xff) |
                   0x80000000u);
    ...
    for (;;) {
      for (unsigned group = 0; group < 2; ++group) {
        uint8_t channel_prefix =
            *(const volatile uint8_t *)(((unsigned long)&rodata_f6bb7) /*=0xf6bb7*/ + group);
        while (pending[group] != 0) {
          ...
          uint16_t settings =
              *(volatile uint16_t *)(((unsigned long)g_gpiote_cb) /*=0x20002bc0*/ + (index + 8) * 2);
