#define G1_APP_USE_LIBLC3_TABLE_DECLS 1
/* Recovered layout bindings (presentation-only; Ghidra-grounded):
 *   param_5          => struct g1_layout_lc3_sns_state__param_0384              [param_0384; G1-original]
 *   local_2e8        => struct g1_layout_lc3_sns_scratch5__stack_1378           [stack_1378; G1-original]
 *   local_2a8        => struct g1_layout_lc3_sns_scratch5_b__stack_1379         [stack_1379; G1-original]
 *   local_268        => struct g1_layout_lc3_sns_analyze_buf__stack_1380        [stack_1380; G1-original]
 *   local_168        => struct g1_layout_lc3_sns_analysis_buffer__stack_1382    [stack_1382; library]
 * Raw function identity: 0x0006c778.  See ../include/g1_recovered_layouts.h. */
#include "g1_app_symbols.h"
/* readable reconstruction; identity: FUN_0006c778 @ 0x0006c778
 * public-name: lc3_sns_analyze
...
#define lc3_sns_mpvq_offsets ((const int32_t (*)[11])((unsigned long)&lc3_sns_mpvq_offsets) /*=0x90128*/)
#define lc3_sns_hfcb ((const float (*)[8])((unsigned long)&lc3_sns_hfcb) /*=0x90450*/)
#define lc3_sns_lfcb ((const float (*)[8])((unsigned long)&lc3_sns_lfcb) /*=0x90850*/)
#define lc3_band_lim ((const int (*)[LC3_NUM_SRATE][LC3_NUM_BANDS+1])((unsigned long)&lc3_band_lim) /*=0x90c50*/)

/* libLC3 fast_log2f(), with its one non-inlined frexpf boundary preserved. */
LC3_HOT static float recovered_fast_log2f(float x)
{
    static const float c[] = {
        -1.29479677f, 5.11769018f, -8.42295281f,
         8.10557963f, -3.50567360f
    };
    int e;
    x = frexpf(x, &e);
    float y = c[0] * x;
    ...
    const float *eb, bool att, float *scf)
{
    ...
    for (int i = 0; i < LC3_NUM_BANDS-1; ) {
        e2 = e[i+1];
        e[i] = __builtin_fmaf(e0, 0.25f,
                   __builtin_fmaf(e1, 0.5f, e2 * 0.25f)) * ge[i];
        e_sum += e[i++];

        e0 = e[i+1];
        e[i] = __builtin_fmaf(e1, 0.25f,
                   __builtin_fmaf(e2, 0.5f, e0 * 0.25f)) * ge[i];
        e_sum += e[i++];

        e1 = e[i+1];
        e[i] = __builtin_fmaf(e1, 0.25f,
                   __builtin_fmaf(e0, 0.5f, e2 * 0.25f)) * ge[i];
        e_sum += e[i++];
    }
    ...
    for (int i = 0; i < LC3_NUM_BANDS; i++)
        e[i] = recovered_fast_log2f(fmaxf(e[i], noise_floor)) * 0.5f;
    ...
    int (*c)[16], float (*cn)[16], int *shape_idx, int *gain_idx)
{
    ...
    __builtin_memcpy(c[2], c[3], sizeof(c[2]));
    ...
    __builtin_memcpy(c[1], c[2], sizeof(c[1]));
    ...
    __builtin_memcpy(c[0], c[1], sizeof(c[0]));
    ...
    *(volatile int *)shape_idx = 0;
    *(volatile int *)gain_idx = 0;

    for (int ic = 0; ic < 4; ic++) {
        const struct lc3_sns_vq_gains *cgains =
            (const struct lc3_sns_vq_gains *)((unsigned long)&lc3_sns_vq_gains) /*=0x903e8*/ + ic;
        ...
        for (int ig = 0; ig < cgains->count; ig++) {
            ...
            float d1 = x[1] - g * cn[ic][1];
            float d0 = x[0] - g * cn[ic][0];
            float mse = __builtin_fmaf(d0, d0, d1 * d1);
            for (int i = 2; i < 16; i++) {
                float d = x[i] - g * cn[ic][i];
                mse = __builtin_fmaf(d, d, mse);
            }
        ...
        if (cmse_min < mse_min) {
            *(volatile int *)gain_idx = cgain_idx;
            *(volatile int *)shape_idx = ic;
            mse_min = cmse_min;
        }
    }
}
 ...
 */
static inline __attribute__((always_inline)) void enum_mvpq(const int *c, int n, int *idx, bool *ls)
{
    int ci, i, j, index = 0;
    ...
    *ls = ci < 0;
    ...
    for (i++, j = LC3_ABS(ci); i < n; i++, j += LC3_ABS(ci)) {
        ...
        index += lc3_sns_mpvq_offsets[i][j];
    }
 ...
 */
static inline __attribute__((always_inline)) void enumerate(int shape, const int *c,
    ...
    const float *x, float *y)
{
    ...
    lc3_sns_unquantize(data->lfcb, data->hfcb,
