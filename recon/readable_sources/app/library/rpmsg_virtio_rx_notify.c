...
void rpmsg_virtio_rx_notify(uint8_t **arg)
{
    ...
    if (*(uint32_t *)(*(uint8_t **)(ctx+0xa0)+0x18)) { firmware_function_000859b2(lock); return; }
    uint32_t *msg=virtqueue_get_buffer(*(void **)(ctx+0xa4),&aux,&n); firmware_function_000859b2(lock);
    while (msg) {
        msg[2]=n; mutex_lock_wait_forever_dup(lock);
        uint8_t *dst=rpmsg_get_endpoint(ctx,0,msg[1],UINT32_MAX); firmware_function_000859b2(lock);
        if (dst) {
            ...
            if (rc<0) { printk(((unsigned long)&rodata_99cbd) /*=0x99cbd*/,((unsigned long)&rodata_f7c0e) /*=0xf7c0e*/,((unsigned long)&rodata_f7ba6) /*=0xf7ba6*/,0x245); assert_post_action(((unsigned long)&rodata_f7ba6) /*=0xf7ba6*/,0x245); }
        }
        mutex_lock_wait_forever_dup(lock);
        if ((int32_t)msg[2]>=0) {
        ...
        if (!msg) goto stop;
        firmware_function_000859b2(lock);
    }
    return;
stop: dma_xfer_end_handler(*(void **)(ctx+0xa4)); firmware_function_000859b2(lock);
}
