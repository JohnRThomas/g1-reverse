 ...
 * Raw function identity: 0x00023634.  See ../include/g1_recovered_layouts.h. */
...
#define FILE_ADDRESS (*(volatile uint32_t *)((unsigned long)&g_font_flash_write_offset) /*=0x200079dc*/)
#define FILE_TYPE (*(volatile int *)((unsigned long)&g_font_total_size) /*=0x200079d4*/)
#define BUFFERED_LENGTH (*(volatile uint32_t *)((unsigned long)&g_font_buf_len) /*=0x200079d8*/)
#define FILE_CHECKSUM (*(volatile uint32_t *)((unsigned long)&g_font_crc32) /*=0x200079e0*/)
#define LOG_LEVEL (*(volatile int *)((unsigned long)&g_log_level) /*=0x2000230c*/)
#define LOG_SINK (*(volatile int *)((unsigned long)&g_log_use_alt_sink) /*=0x20007554*/)
#define FILE_BUFFER ((uint8_t *)((unsigned long)&g_font_write_buf) /*=0x20017062*/)
...
void try_to_save_file(int context)
{
    ...
    if (packet_length < 0x15 && packet[0] == 0x0d && packet[1] == 0x0e) {
        ...
        if (BUFFERED_LENGTH < 0x1000) {
            ...
            log_message(((unsigned long)&rodata_9e88b) /*=0x9e88b*/, ((unsigned long)&rodata_9e8e5) /*=0x9e8e5*/, checksum);
            return;
        }
        goto save_block;
    }
...
save_block:
    ...
    if (LOG_LEVEL > 0) {
        if (LOG_SINK == 0)
            log_message(((unsigned long)&rodata_9e873) /*=0x9e873*/, ((unsigned long)&rodata_9e8e5) /*=0x9e8e5*/, checksum);
        else
            debug_print();
    }
    FILE_ADDRESS += 0x1000;
