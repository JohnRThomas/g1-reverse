...
extern void z_log_msg_runtime_create(u32 sink, u32 code, const void *record, u32 context);
extern int bt_hci_cmd_create(u32 key, u32 size);
extern int bt_hci_cmd_send_sync(u32 key, int object, u32 flags);
                  ...
                  int argument5, u32 argument6)
{
    ...
    uint64_t lookup = deref_conn_field0((const void *)((unsigned long)&g_ble_dev_ncmd_sem) /*=0x200020d4*/);
    u32 lookup_status = (u32)lookup;
    u8 *resource = (u8 *)(uintptr_t)(lookup >> 32);
    if ((lookup_status & 4u) == 0)
        return -11;

    if (ble_conn_addr_and_params_valid(resource, resource, argument3, 0) == 0)
        return -22;
    int state = normalize_to_bool(resource);
    if (state == 0)
        return -22;

    u8 *fields = output + 0x10;
    u32 field_status = (u32)deref_conn_field0(fields);
    ...
    atomic_and_1((const void *)((unsigned long)&g_ble_dev_ncmd_sem) /*=0x200020d4*/);

    u8 resource_id = *(volatile u8 *)resource;
    *output = resource_id;
    *(volatile u8 *)((unsigned long)&g_2000206f) /*=0x2000206f*/ = resource_id;

    int result = bt_id_set_adv_random_addr(output, resource_flags, initial_mode,
                              record.details);
    if (result != 0)
        return result;
    ...
    int kind;
    if (configured == 0) {
        bt_addr_le_copy(output + 9, ((unsigned long)&rodata_f2b3a) /*=0xf2b3a*/);
        ...
        bt_addr_le_copy(output + 9, *(volatile u32 *)(resource + 0x10));
        kind = get_adv_name_type_param(resource);
        resource_flags = *(volatile u32 *)(resource + 4);
        if ((resource_flags & 1u) != 0) {
            record.mode = (resource_flags & 0x10u) == 0 ? 1 : 4;
            bt_addr_le_copy(&record.details[1],
    ...
    if (object15 == 0)
        return -105;
    net_buf_simple_add_mem((void *)(uintptr_t)(object15 + 0x0c), &record, 15);
    result = bt_hci_cmd_send_sync(0x2006, object15, 0);
    if (result != 0)
        return result;
    ...
    if (result != 0) {
        const struct error_record error = {2, ((unsigned long)&rodata_f381d) /*=0xf381d*/};
        z_log_msg_runtime_create(((unsigned long)&rodata_880f8) /*=0x880f8*/, 0x1040, &error, 0);
        if (allocation != 0) {
...
publish_fields:
    ...
    atomic_assign_bit(fields, 14, alternate);
    atomic_assign_bit(fields, 8, kind == 1);
    atomic_assign_bit(fields, 9, kind == 2);
    atomic_assign_bit(fields, 10, resource_flags & 1u);
    atomic_assign_bit(fields, 11, (u32)state);
    atomic_assign_bit(fields, 13, (resource_flags >> 2) & 1u);
    return 0;
}
