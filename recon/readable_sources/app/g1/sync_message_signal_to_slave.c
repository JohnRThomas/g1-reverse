...
unsigned int sync_message_signal_to_slave(unsigned int param_1, unsigned int param_2, unsigned int param_3)
{
    ...
    if (*(volatile unsigned int*)((unsigned long)&g_log_use_alt_sink) /*=0x20007554*/ == 0) {
        log_message(uVar4, ((unsigned long)"sync_message_signal_to_slave") /*=0xa3a24*/);
    } else {
        debug_print(uVar4, ((unsigned long)"sync_message_signal_to_slave") /*=0xa3a24*/);
    }
    uVar8 = k_uptime_get_2();
    if ((long long)(lVar7 + 3000) < (long long)uVar8) {
        if (*(volatile unsigned int*)((unsigned long)&g_log_use_alt_sink) /*=0x20007554*/ == 0) {
            log_message(((unsigned long)"retry sync_to_slave failed !") /*=0xa26ae*/);
        } else {
            debug_print(((unsigned long)"retry sync_to_slave failed !") /*=0xa26ae*/);
        }
    }
    return uVar2;
}
