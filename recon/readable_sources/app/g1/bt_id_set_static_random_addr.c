...
int bt_id_set_static_random_addr(int index, int handle, unsigned int value)
{
    ...
    if (handle != 0 && bt_addr_le_eq(handle, ((unsigned long)&rodata_f2b3a) /*=0xf2b3a*/) == 0) {
        bt_addr_le_copy_80fc4(((unsigned long)&g_ble_dev_state) /*=0x20002000*/ + index * 7, handle);
    } else {
        do {
        ...
        result = bt_addr_le_copy_80fc4(((unsigned long)&g_ble_dev_state) /*=0x20002000*/ + index * 7,
                              (int)&operation);
        if (handle != 0)
            bt_addr_le_copy_80fc4(handle, result);
    }

    result = ptr_read_u32(((unsigned long)&g_ble_dev_ncmd_sem) /*=0x200020d4*/);
