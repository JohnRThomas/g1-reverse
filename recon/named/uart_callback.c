...
void uart_callback(int param_1,unsigned char *param_2,unsigned int param_3){
  ...
  switch (*param_2) {
  case 0:
    if (2 < *lvl) { if(*g8==0) log_message(((unsigned long)"%s(): Tx sent %d bytes\n"),((unsigned long)"uart_callback"),VI((int)param_2+8),0,param_1); else debug_print(((unsigned long)"%s(): Tx sent %d bytes\n"),((unsigned long)"uart_callback"),VI((int)param_2+8),0,param_1); }
    for (uVar5=0; uVar5 < (unsigned int)VI((int)param_2+8); uVar5++)
      log_message(0xa7993, VB(VI((int)param_2+4)+uVar5));
    break;
  case 1:
    if (*lvl < 1) return;
    if (*g8 != 0) { debug_print(((unsigned long)"%s(): Tx aborted\n"),((unsigned long)"uart_callback")); return; }
  ...
  case 2:
    ...
    log_message(((unsigned long)"recv data %d bytes:"), VI((int)param_2+0xc), param_3, uVar7 & 0xffff, param_1);
    for (uVar7=0; uVar7 < (unsigned int)VI((int)param_2+0xc); uVar7++)
      log_message(0xa7993, VB(VI((int)param_2+4)+uVar5+uVar7));
    break;
  case 3:
