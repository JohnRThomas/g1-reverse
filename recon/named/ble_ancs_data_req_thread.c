...
void ble_ancs_data_req_thread(uint8_t *c,uint32_t p2,uint32_t p3,uint32_t p4){
 ...
 z_impl_k_sem_init(c+0x200,0,10);z_impl_k_sem_init(c+0x1e8,0,10);init_msgq_ancs();init_msgq_uid();wait_for_event(0x28000,0);
 for(;;){uint8_t*g=get_device_info();if(g[1]==1){wait_for_event(0x28000,0);continue;}g=get_device_info();if(g[1]==8){wait_for_event(0x28000,0);continue;}g=get_device_info();
  if((*(uint16_t*)(g+0x105c)&0x80)==0){k_msleep_ticks32768_d(1000);continue;}
  if(*lv>0){if(!*sink)log_message(((unsigned long)"%s(): !!!!!!!!!wait -> ancs_get_attr_req_sem\n"),((unsigned long)"ble_ancs_data_req_thread"));else debug_print(((unsigned long)"%s(): !!!!!!!!!wait -> ancs_get_attr_req_sem\n"),((unsigned long)"ble_ancs_data_req_thread"));}z_impl_k_sem_take(c+0x200,-1LL);
  if(dequeue_uid((void*)ev)==0){if(*lv>0){if(!*sink)log_message(((unsigned long)"%s(): !!!!!!!!!!!!!!get -> ancs_get_attr_req_sem %d evt_id %d\n"),((unsigned long)"ble_ancs_data_req_thread"),*ev,((uint8_t*)ev)[4]);else debug_print(((unsigned long)"%s(): !!!!!!!!!!!!!!get -> ancs_get_attr_req_sem %d evt_id %d\n"),((unsigned long)"ble_ancs_data_req_thread"),*ev,((uint8_t*)ev)[4]);}
   if(((uint8_t*)ev)[4]<2){(void)request_ancs_attr_ext();if(*lv>0){if(!*sink)log_message(((unsigned long)"%s(): !!!!!!!!!!!!wait -> ancs_get_attr_req_sem add done, and wait -> ancs_get_attr_data_sem\n"),((unsigned long)"ble_ancs_data_req_thread"));else debug_print(((unsigned long)"%s(): !!!!!!!!!!!!wait -> ancs_get_attr_req_sem add done, and wait -> ancs_get_attr_data_sem\n"),((unsigned long)"ble_ancs_data_req_thread"));}z_impl_k_sem_take(c+0x1e8,0x8000LL);}
   else{*(uint32_t*)(c+0x1e4)=2;*(uint32_t*)(c+0x3c)=*ev;c[0x41]=0;c[0x44]=0x30;c[0x64]=0x30;c[0x42]=0;enqueue_ancs(c+0x34);memset_bytes(c+0x34,0,0x1b4);if(*lv>0){if(!*sink)log_message(((unsigned long)"%s(): !!!!!!!!!!!!wait -> ancs_get_attr_req_sem remove done, and wait -> ancs_get_attr_data_sem\n"),((unsigned long)"ble_ancs_data_req_thread"));else debug_print(((unsigned long)"%s(): !!!!!!!!!!!!wait -> ancs_get_attr_req_sem remove done, and wait -> ancs_get_attr_data_sem\n"),((unsigned long)"ble_ancs_data_req_thread"));}}
   ancs_notification_forward(c);if(*lv>0){if(!*sink)log_message(((unsigned long)"%s(): !!!!!!!!!!!!!!!!wait -> ancs_get_attr_data_sem done\n"),((unsigned long)"ble_ancs_data_req_thread"));else debug_print(((unsigned long)"%s(): !!!!!!!!!!!!!!!!wait -> ancs_get_attr_data_sem done\n"),((unsigned long)"ble_ancs_data_req_thread"));}}
 }}
