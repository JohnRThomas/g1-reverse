...
unsigned process_pt_data(unsigned param_1)
{
    ...
    if (iVar3 != 0) { log_message(0x000a7780); nfc_eeprom_comm_unlock(); return 0xfffffffd; }

    channel = *(volatile int *)0x20002424u;
    if (mode != 0)
        channel = *(volatile int *)0x20002428u;
    iVar3 = pt_comm_transport_write(mode, channel, local_1c, local_28 & 0xff);
    if (iVar3 != 0) { log_message(0x000a779f); nfc_eeprom_comm_unlock(); return 0xfffffffc; }
    ...
    if (local_1c[9] == 0x80) {
        ...
        local_1c[9] = 0;
        nfc_eeprom_comm_lock();
        pt_comm_transport_write(mode, *(volatile int *)0x20002424u,
    ...
    if (uVar4 == 0x1b) {
        ...
        if (uVar4 != 6) {
            ...
            do {
                if (pbVar5 == 0) log_message(0x000a77cc, 0x000a7882);
                else debug_print(0x000a77cc, 0x000a7882);
LAB_0003324c:
                do {
                    while (1) {
...
LAB_0003325a:
                        ...
                        if (0 < (int)pbVar5) {
                            pbVar5 = (unsigned char *)*(volatile int *)0x20007554UL;
                            if (pbVar5 == 0) log_message(0x000a780d, 0x000a7882);
                            else debug_print(0x000a780d, 0x000a7882);
                        }
                    }
                    if (uVar4 != 1) return 0;
                    if (pbVar5[4] != 0) return 0;
