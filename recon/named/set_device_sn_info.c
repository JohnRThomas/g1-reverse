...
uint32_t set_device_sn_info(int param_1, uint32_t param_2, uint32_t *param_3, uint8_t *param_4){
    ...
    if (param_3==0 || param_4==0){ log_message(((unsigned long)"get_query_sn_info para is NULL\n")); uVar1=0xffffffff; }
        ...
        if (2 < *(volatile int*)0x2000230cUL){
            if (*(volatile int*)0x20007554UL == 0){ uVar1=get_device_serial_buf(); log_message(((unsigned long)"%s(): BLE_REQ_PUT_DEVICE_SN new sn:%16s\n"),((unsigned long)"set_device_sn_info"),uVar1); }
