/* named: FUN_0005b0dc */
/* Reconstructed FUN_0005b0dc @ 0x5b0dc  (parity: 296/300 trials, PROVEN) */
extern int  bt_gatt_attr_next(int);
extern void assert_post_action(int,int);
extern void printk(int,int,int,int);
extern int  bt_uuid_cmp(int,void*);
extern int  bt_conn_is_peer_addr_le(int,int,void*);
#define VI(a) (*(volatile int*)(a))
#define VB(a) (*(volatile unsigned char*)(a))
#define VH(a) (*(volatile unsigned short*)(a))

int FUN_0005b0dc(int param_1,int param_2,unsigned int param_3,unsigned int param_4){
  int iVar2; int buf[4];
  (void)param_4;
  if (param_1 == 0) { printk(0,0,0,0);printk(0,0,0,0);assert_post_action(0,0xd19); return 0; }
  if (param_2 == 0) { printk(0,0,0,0);printk(0,0,0,0);assert_post_action(0,0xd1a); return 0; }
  if (VB(param_1+0xd) != 7) return 0;
  iVar2 = bt_uuid_cmp(VI(param_2), buf);
  if (iVar2 == 0) {
    if ((VB(VI(param_2+0xc)+6) & 0x30) == 0) return 0;
    param_2 = bt_gatt_attr_next(param_2);
    if (param_2 == 0) { printk(0,0,0,0);printk(0,0,0,0);assert_post_action(0,0xd2b); return 0; }
  }
  iVar2 = bt_uuid_cmp(VI(param_2), buf);
  if (iVar2 != 0) {
    param_2 = bt_gatt_attr_next(param_2);
    if (param_2 == 0) { printk(0,0,0,0);printk(0,0,0,0);assert_post_action(0,0xd31); return 0; }
  }
  for (;;) {
    iVar2 = bt_uuid_cmp(VI(param_2), buf);
    if (iVar2 == 0) goto LAB_1ca;
    iVar2 = bt_uuid_cmp(VI(param_2), buf);
    if (iVar2 == 0) goto LAB_1ca;
    iVar2 = bt_uuid_cmp(VI(param_2), buf);
    if (iVar2 == 0) goto LAB_1ca;
    iVar2 = bt_uuid_cmp(VI(param_2), buf);
    if (iVar2 == 0) goto LAB_1ca;
    param_2 = bt_gatt_attr_next(param_2);
    if (param_2 == 0) return 0;
  }
LAB_1ca:
  iVar2 = bt_uuid_cmp(VI(param_2), buf);
  if (iVar2 != 0) return 0;
  {
    int puVar4 = VI(param_2+0xc);
    iVar2 = bt_conn_is_peer_addr_le(param_1, VB(puVar4), (void*)(puVar4+1));
    if (iVar2 == 0) return 0;
    return (param_3 & VH(puVar4+8)) != 0;
  }
}

