...
void bt_gatt_connected(uint8_t *connection)
{
    ...
    if (bt_addr_le_is_bonded(connection[8], peer)) {
        ...
        bt_settings_encode_key(key, sizeof(key), (const char *)0x000f4aa1U,
                     peer, identity_component);
        settings_load_subtree_direct(key, (void *)0x0005af8dU, key);
    }
    bt_gatt_foreach_attr_0(1U, 0xffffU, (void *)0x0005b555U, &data);
    if (bt_conn_get_security(connection) < data.security) {
        ...
        if (error != 0) {
            ...
            log_msg_create_3arg(0x00088128U, 0x1880U, package);
        }
    }
}
