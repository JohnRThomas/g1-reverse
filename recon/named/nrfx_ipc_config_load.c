...
void nrfx_ipc_config_load(int param_1,uint32_t param_2,uint32_t param_3,uint32_t param_4){
  if(param_1==0){
    printk(((unsigned long)"ASSERTION FAIL [%s] @ %s:%d\n"),((unsigned long)"p_config"),0xf6bc5,0x46,param_4);
    assert_post_action(0xf6bc5,0x46);
  }
 L:
  if(*(volatile uint8_t*)(0x2000b324UL+4)==1){
    int iVar5=0;
    do{
      int iVar1=iVar5*4;
  ...
  printk(((unsigned long)"ASSERTION FAIL [%s] @ %s:%d\n"),((unsigned long)"m_cb.state == NRFX_DRV_STATE_INITIALIZED"),0xf6bc5,0x47,param_4);
