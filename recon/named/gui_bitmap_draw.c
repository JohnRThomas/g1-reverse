...
unsigned gui_bitmap_draw(uint param_1,int param_2,int param_3,int param_4,int param_5,byte param_6)
{
    ...
    if((param_3 < (int)param_1) || (param_4 < param_2) || (0xf < uVar9)){
        ...
        if((param_1 & 1) == 0){
            ...
            if((uVar14 & 2) != 0){
                clean_fb_data(*(volatile int*)0x2000a034, 0, param_1 - 2, param_2, iVar7 + 2, param_4);
            }
    ...
    if(iVar7 == 0){ log_message(uVar10, ((unsigned long)"gui_bitmap_draw")); }
    else { debug_print(uVar10, ((unsigned long)"gui_bitmap_draw")); }
    return 0xffffffff;
}
