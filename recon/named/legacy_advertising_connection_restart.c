...
void legacy_advertising_connection_restart(void)
{
    ...
    if (ble_conn_addr_resolve_or_create((void *)0x20002018UL, &connection) != 0) return;
    {
        uint32_t directed = adv_is_directed((void *)0x20002018UL);
        ...
        {
            int error = bt_id_set_adv_random_addr((void *)0x20002018UL, mode,
                                      directed, &option);
            if (error != 0) {
                uint32_t package[3] = { 3U, 0x000f3857UL, (uint32_t)error };
                z_log_msg_runtime_create(0x000880f8UL, 0x1840U, package, 0U);
                return;
            }
        }

        if (bt_le_adv_set_enable_legacy((void *)0x20002018UL, 1U) == 0) {
