...
void imu_fusion_thread(char *p)
{
    ...
    L102e0:
        ...
        if (p[0] == 2) {
            ...
            I(0x20007a8c) = -3;
            B(0x20018d96) = 1;
            if (I(0x20007554) == 0) log_message(((unsigned long)"bow head################################################\n")); else debug_print(((unsigned long)"bow head################################################\n"));
            send_event(3);
            p[0] = 1;
            {
                int e = p[-0xe0f];
