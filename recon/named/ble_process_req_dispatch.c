...
unsigned int ble_process_req_dispatch(uint8_t *param_1, uint8_t *packet, uint8_t *param_3)
{
    ...
    if ((uint32_t)(command_id - 0x47u) > 9u) {
        ...
        transfer[1] = 0xca;
        transfer[2] = 0x15;
        strcpy(transfer + 2, (const void *)0x0009d671u);
        {
            response_callback_t pc = *(response_callback_t *)(param_1 + 0xc);
            pc(transfer, 20);
        }
    ...
    default:
        ...
        response[1] = 0xca;
        response[2] = 0x15;
        strcpy(response + 2, (const void *)0x0009d671u);
        break;
    case 0x49:
    case 0x4d:
...
switchD_caseD_49:
    ...
    case 0x4e:
        if (load_i16(packet + 2) != 0) {
            memset_bytes((void *)0x2000ffa7u, 0, 0x158);
            ...
            if (*pcVar8 == 1) {
                if (cVar1 == 1) {
                                            ...
                                            __ATOMIC_ACQ_REL);
                    {
                        ...
                        z_impl_k_timer_start(0x20003d28u, cleared, 0x8000u,
                                     0, 0, 0);
                    }
                    return 0;
                }
