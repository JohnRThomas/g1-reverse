...
extern uintptr_t sys_notify_finalize(void *notify, int32_t result);
void notify_one(void *manager, void *client, uint32_t state, int32_t result)
{
    onoff_client_callback callback = (onoff_client_callback)sys_notify_finalize((uint8_t *)client + 4u, result);
    if (callback != 0) {
