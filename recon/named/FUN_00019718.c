...
void FUN_00019718(uint8_t *c,uint32_t p2,uint32_t p3,uint32_t p4){
 ...
 if(*lv>2){if(!*sink)log_message(0x99969,0x9b0d8,p3,0,p4);else debug_print(0x99969,0x9b0d8,p3,*sink,p4);}
 ...
 for(;;){uint8_t*g=get_device_info();if(g[1]==1){wait_for_event(0x28000,0);continue;}g=get_device_info();if(g[1]==8){wait_for_event(0x28000,0);continue;}g=get_device_info();
  ...
  if(*lv>0){if(!*sink)log_message(0x9adc7,0x9b0d8);else debug_print(0x9adc7,0x9b0d8);}z_impl_k_sem_take(c+0x200,-1LL);
  if(dequeue_uid((void*)ev)==0){if(*lv>0){if(!*sink)log_message(0x9adf5,0x9b0d8,*ev,((uint8_t*)ev)[4]);else debug_print(0x9adf5,0x9b0d8,*ev,((uint8_t*)ev)[4]);}
   if(((uint8_t*)ev)[4]<2){(void)request_ancs_attr_ext();if(*lv>0){if(!*sink)log_message(0x9ae34,0x9b0d8);else debug_print(0x9ae34,0x9b0d8);}z_impl_k_sem_take(c+0x1e8,0x8000LL);}
   else{*(uint32_t*)(c+0x1e4)=2;*(uint32_t*)(c+0x3c)=*ev;c[0x41]=0;c[0x44]=0x30;c[0x64]=0x30;c[0x42]=0;enqueue_ancs(c+0x34);memset_bytes(c+0x34,0,0x1b4);if(*lv>0){if(!*sink)log_message(0x9ae92,0x9b0d8);else debug_print(0x9ae92,0x9b0d8);}}
   FUN_0001965c(c);if(*lv>0){if(!*sink)log_message(0x9aef3,0x9b0d8);else debug_print(0x9aef3,0x9b0d8);}}
 }}
